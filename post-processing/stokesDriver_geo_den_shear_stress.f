...
c      parameter (noutc = 2**8, nouter = nbeta*noutc)      
      ...
      dimension shear_stress(ninner*nmax)
...
!      if ( ibary .eq. 1) then
!          if( ifmm .eq. 1) then 
!             open(unit=1,file='record/xtar_fmmbary.dat')          
!             open(unit=2,file='record/ytar_fmmbary.dat')          
!          else
...
!      else
!        if( ifmm .eq. 1) then
!          open(unit=1,file='output/xtar_fmm.dat')      
!          open(unit=2,file='output/ytar_fmm.dat')      
!        else
...
!        open(unit=1,file='geo_20b_dense/geom.dat')  
        open(unit=1,file='run_params/geom'//trim(f2)//'.dat')
        do i=1,3
          read(1,*)
        enddo
        read(1,1020) nbodies      
...
!        open(unit=2,file='den_20b_dense/density0100.dat')
        open(unit=2,file='run_params/density'//trim(f2)//'.dat')
...
!          open(unit=1,file='record/nbody.dat',access = 'append')
!          open(unit=8,file='record/x'//trim(f1)//'.dat')
!          open(unit=9,file='record/y'//trim(f1)//'.dat')
...
!          open(unit=8,file='output/x_fmm.dat')      
!          open(unit=9,file='output/y_fmm.dat')
!        else   
!          open(unit=8,file='output/trap_x.dat')      
!          open(unit=9,file='output/trap_y.dat')      
!        endif
!        write(1,1020) nbodies
...
!        
  ...
  !      if ( ibary .eq. 1 .and. ifmm .eq. 1) then
          open(unit=2,file='record/shear_stress'//trim(f1)//'.dat')
  !        open(unit=3,file='record/pressure_fmmbary.dat')
  !        open(unit=4,file='record/drag_fmmbary.dat')
  !      elseif (ibary .eq. 1 .and. ifmm .eq. 0) then  
...
!        if ( ibary .eq. 1) then
!          if( ifmm .eq. 1) then          
!            open(unit=3,file='record/utar'//trim(f1)//'.dat')          
!            open(unit=4,file='record/vtar'//trim(f1)//'.dat')
!            open(unit=8,file='record/press_tar'//trim(f1)//'.dat')            
!!            open(unit=8,
!!     &       file='output/record/drop_press'//trim(f1)//'.dat')          
!            open(unit=9,file='record/vort_tar'//trim(f1)//'.dat')
...
!        else
!          if( ifmm .eq. 1) then   
!            open(unit=3,file='output/utar_fmm.dat')      
!            open(unit=4,file='output/vtar_fmm.dat')      
!            open(unit=8,file='output/press_tar_fmm.dat')      
!            open(unit=9,file='output/vort_tar_fmm.dat') 
!          else   
