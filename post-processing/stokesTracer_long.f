...
c      open(unit=1,file='geo_100b/geom0100.dat')      
      ...
      open(unit=2,file='density0001.dat')
...
c     if the previos iteration is not enough
      if (irec . eq. 1) then
        open(unit=4,file='xtracer_final1.txt')    
        open(unit=5,file='ytracer_final1.txt')
        do k = 1,ntracer
          read(4,1000) xtrac(k)     
          read(5,1000) ytrac(k)
        enddo     
        ...
        open(unit=4,file='xtracer1.txt',
     $   access='append')  
        open(unit=5,file='ytracer1.txt',
     $   access='append')
        do k = 1,ntracer
...
c        print *, xtrac(1)
     ...
     $    ntracer,xtrac,ytrac,utrac,vtrac,press_trac,vort_trac)
      ...
      open(unit=4,file='xtracer1.txt',access='append')
c     $       ,status='replace')     
      open(unit=5,file='ytracer1.txt',access='append')
c     $       ,status='replace')
      open(unit=6,file='utracer1.txt',access='append')
c     $       ,status='replace')      
      open(unit=7,file='vtracer1.txt',access='append')
c     $       ,status='replace')
...
c      find the closest inlet velocities to begin with when the tracers hit the outlet             
           if(xtrac(k) .GT. xmax .OR. dabs(ytrac(k)) .GT. 1.d0) then
            open(unit=4,file='xtracer1.txt')
            open(unit=5,file='ytracer1.txt')
            open(unit=6,file='utracer1.txt')  
            open(unit=7,file='vtracer1.txt')
            do j = 1,ntracer
              read(4,1000) xin(j)
              read(5,1000) yin(j)            
              read(6,1000) uin(j)
...
c     record the last position of tracer if the iteration is not enough        
        open(unit=8,file='xtracer_final1.txt')
c     $       ,status='replace')     
        open(unit=9,file='ytracer_final1.txt')
c     $       ,status='replace')       
        do k = 1,ntracer
