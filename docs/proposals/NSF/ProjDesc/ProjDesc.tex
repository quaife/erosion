...
We propose to extend our preliminary results for pure advection so that
concentrations with finite Peclet numbers can be investigated.
Numerical methods to solve the advection-diffusion
equation~\eqref{eqn:advectionDiffusion} in a complex geometry poses
several challenges that we will address.  We will start by splitting the
advection and diffusion equations using a Strang splitting~\cite{}.
Numerical solutions of the advection equation will be formed with a
semi-Lagrangian method. These methods have the advantage that they ...?
The forced diffusion equation is more challenging to solve in a complex
geometry with an integral equation formulation. One particular challenge
is extending the forcing function from the complex geometry and into a
...
basis functions to form a sufficiently smooth extension~\cite{}
\todo[inline]{A-K, MC}. Alternatively, others have used the space-time
heat kernel~\cite{}, but this requires storing an entire time history of
the solution. We propose to develop a new method that uses the Laplace
transform.
