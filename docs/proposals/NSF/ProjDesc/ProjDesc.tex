...
At the finest scale, the fluid flow intervening between individual grains of a porous medium is governed by the incompressible Stokes equations \begin{equation}
\label{eqn:StokesEq}
\begin{split}
...
\begin{itemize}[noitemsep]
  \item Computational plays an essential role to simulate erosion of porous medium.

  \item The proposed research is directly applicable to several geophysical applications.

  \item The proposed research is a collaborative effort and will include graduate, undergraduate, and K--12 students
\end{itemize}

\paragraph{Results from Prior NSF Support:} Neither PI has previous funding from NSF.

\paragraph{PIs' Qualifications:} PI Moore joined the Department of Mathematics at Florida State University (FSU) in 2014, and is also a Research Associate in FSU's Geophysical Fluid Dynamics Institute (GFDI).  His expertise includes modeling dynamic interactions between fluids and structures, especially in geophysical and biologically motivated settings. PI Quaife joined the Department of Scientific Computing at FSU in 2015 and is also a Research Associate in GFDI. His expertise includes high-fidelity and efficient numerical methods for fluid dynamic in complex geometries.

The PIs co-authored a paper describing numerical methods for simulating erosion~\cite{Quaife2018} in a viscous fluid. Very recently, a second paper has been submitted to the Journal of Fluid Mechanics~\cite{chi-moo-qua2019}, where additional numerical methods are developed and characterizations of transport are presented.
...
%----------------------------------------------------------------------%
\todo[inline]{Discuss what the main research questions are here}
\begin{itemize}[noitemsep]
...
Near-term plans are to include flow transport and gravitational sedimentation of the eroding grains. For this extension, contact forces between bodies must be included to eliminate unphysical contact that is inevitable due to discretization errors. 
