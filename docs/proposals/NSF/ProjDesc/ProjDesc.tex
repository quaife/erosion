...
In addition to the work of the PIs, dynamic erosion of a porous media is a highly active subject as laboratory experiments on erosion of colloidal systems are currently being explored in the Datta Lab at Princeton~\cite{bizmark2019multiscale}. These experiments are closely related to our proposed computational study, thus offering a great opportunity for comparison. Furthermore, {\bf we propose performing a set of experiments conducted at the nearby Geophysical Fluid Dynamics Institute (see Section~\ref{sec:experiments}), which would help to guide, calibrate, and validate the computations (see letter of collaboration).} 
...
\todo[inline]{The last paragraph of this sections seems out of place.  Maybe it just needs to be reversed with the one before it?}
%----------------------------------------------------------------------%
...
\begin{wrapfigure}[19]{r}{0.3\textwidth}
  ...
  \caption{\label{fig:100tracers} \em Streamlines visualized with 200 tracers passing through an eroded geometry. The tracers are initialized at the left end of the channel and the flow is from left to right.} \end{wrapfigure}
\begin{align}
...
\todo[inline]{We have more room at the end of the document. This
section seems a bit brief and could maybe be expanded.}
