...
the result is a high-fidelity numerical simulation of near-optimal
...
\end{align}
where $D$ is \nick{\st{an} a} kernel of the integral operator, $\rr =
\xx - \yy$, $\rho = \|\rr\|$, $\nn$ is the unit outward normal at $\yy$,
and $\eeta$ is an unknown density function.  The double-layer potential
can not represent all solutions of the incompressible Stokes equations,
so we complete the integral equation formulation by adding the $M$
...
\end{align}
must be added to $\ssigma(\xx)$ for $\xx \in \bd\Omega$.
Finally, the Stokeslets and rotlets do not exhibit any jumps
across $\gamma$, and so the associated components of the deformation tensor, pressure, and vorticity due to the Stokeslets
