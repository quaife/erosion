...
\label{sec:intro}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Porous media flow plays an important role in several environmental and
industrial applications.  To understand flow in a porous media, it is
important to accurately characterize the length scales and velocity
scales. However, this is very challenging since for a single porous
geometry, the length scales that can vary from $10^{-6}m$ to
$10^{-1}m$~\cite{mil-chr-imh-mcb-ped1998}, and the velocity scales that
can vary from $10^{-1}m/$day to $10m/$day~\cite{kut-scr-dav-ham1995}.
However, resolving all these scales is fundamental to develop meaningful
constitutive relationships that link the microscopic to the macroscopic
...
Coarse-grained variables are often used to study flow in a porous media,
and examples include pressure-saturation-permeability
...
Flow in porous media is often studied for a fixed geometry, but for many
physical processes, the geometry evolves with the flow. For example,
melting~\cite{}\todo[inline]{Natural convection solid/liquid phase
change in porous media}, dissolution~\cite{kan-zha-che-he2002},
deposition~\cite{joh-eli1995}, and biofilm
growth~\cite{tan-val-wer2015}.  Here, we focus on erosion which is
fluid-mechanical process that is prevalent in many geophysical and
hydrological applications.  For example, contaminant transport through
aquifers~\cite{cve-che-wen1996} and groundwater~\cite{dag1987,
kon-bre1978} is largely effected by the nature of the eroded porous
geometry.  Porous geometries that have undergone erosion have
...
results in velocity scales that vary over several orders of
magnitude~\cite{all-hea-lab-rei2002} (see
Figure~\ref{fig:Eroding50vel}).  Moreover, the channelization results in
a geometries that are largely heterogeneous and anisotropic, and this
affects the transfer other other quantities such as a contaminant or
heat~\cite{nil-sto1990, ree-sto1995}.  Others have studied transport in
heterogeneous geometries (for example,
...
fluid, the grains of the porous media are eroded at a rate that
is proportional to the hydrodynamic shear stress~\cite{wan-fel2004,
ris-moo-chi-she-zha2012, par-izu2000}.  Porous geometries
that have undergone erosion differ from standard porous geometry because
of large amounts of anisotropy and channelization. This work
addresses the effects that erosion has on the transport through a porous
media.  Our contributions are two-fold. First, starting with our
previous work~\cite{qua-moo2018}, we develop new numerical methods to
simulate erosion of geometries with low porosities and nearly-touching
grains. Second, we compute and analyze three different metrics of
transport throughout the erosion process: the tortuosity, the anomalous
dispersion, and the pore size distribution.

To calculate the tortuosity and dispersion
...
\begin{align}
  ...
  \label{eqn:tracers}
\end{align}
where $\uu$ is the fluid velocity.  Tracer trajectories can be simulated
with a continuous time random walks~\cite{mon-wei1965,
den-cor-sch-ber2004, leb-den-car2008, ber-cor-den-sch2006} or with a
Fokker-Planck equation~\cite{ica-den2019}.  Instead, since there is no
stiff diffusive term in~\eqref{eqn:tracers}, we compute streamlines by
applying a high-order explicit Runge-Kutta time stepping method.
...
\begin{figure}[H]
\begin{center}
\todo[inline]{plots of the magnitude of the velocity on a log based 10
scale}
\includegraphics[width = 0.3 \textwidth]{./figs/vel_log_50b1}
\includegraphics[width = 0.3 \textwidth]{./figs/vel_log_50b80}
\includegraphics[width = 0.37 \textwidth]{./figs/vel_log_50b160}\\
\includegraphics[width = 0.3 \textwidth]{./figs/vel_log_50b240}
\includegraphics[width = 0.3 \textwidth]{./figs/vel_log_50b320}
\includegraphics[width = 0.37 \textwidth]{./figs/vel_log_50b400}
\caption{\label{fig:Eroding50vel} Erosion of 50 nearly touching bodies
with the fixed pressure drop condition. The color is the velocity of
the fluid in log scale. The 6 snapshots are evenly spaced in time.}
\end{center}
\end{figure}
