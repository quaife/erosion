...
several equispaced time steps.  For the three initial configurations,
the shear stress is positive for all time, but varies over several
orders of magnitude.  Therefore, we color eroding body's boundary
according to the logarithm of the shear stress.  Comparing our results
to Mitchell and Spagnolie~\cite{mit-spa2017} (Figure 7(c)), we see that
the body shapes are similar.  The differences we can attribute to our
assumption of a two-dimensional flow.  In three dimensions, there is no
recirculation between the solid wall and the spherical
body~\cite{cha-feu2003}.  In contrast, between a two dimensional
circular body and a solid wall, there are regions of recirculation on
the fore and aft sides of the body.  In Figure~\ref{fig:NearWall_vort},
we plot the vorticity at the final configuration, and the where the
vorticity is smallest corresponds to the region where there is a slow
recirculation.  

Since the shear stress is always positive, the erosion rate is a smooth
function and does not lead to the development of corners.  However,
since the location where the shear stress is minimized is close to
regions of high shear stress, the rate of erosion changes quickly, and
the result is a body that seems to be developing a corner, although it
is not.  Similar to the three dimensional case~\cite{mit-spa2017}, this
region of high curvature develops in the top half of the eroding body.
...
\approx \left(\frac{t}{t_f}\right)^{3.43}, \end{align}
...
\begin{center}
  ...
  \includegraphics[height=0.22\textwidth]{./figs/20b_dense301}
