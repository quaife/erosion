...
The first method to compute numerically compute the tortuosity uses $N_p$ trajectories
that are initially at $(1,y_i)$, $i=1,\ldots,N_p$, and stopped when
their $x$ coordinate is 1.  T

with originating at $x=-1$ and terminating at $x=1$, we use the approximation 
\begin{align}
...
Similarly, to compute the tortuosity in terms of the Eulerian velocity
field, we use the approximation
\begin{align}
  T = \frac{\sum\limits_{i,j=1}^N \|\uu(\yy_{i,j})\|\Delta x_i \Delta y_j}
      {\sum\limits_{i,j=1}^N u(\yy_{i,j}) \Delta x_i\Delta y_j},
\end{align}
where $\Delta x_i = 1/N$
