...
recall that the outer wall is bounded in $[-3,3] \times [-1,1]$, and
that the bodies are all contained in $[-1,1] \times [-1,1]$.  Therefore,
a target point can only be too close to the lines $y = \pm 1$, so we
first check if
\begin{align}
  \left\|\cc_i - \left[
    \begin{array}{c}
