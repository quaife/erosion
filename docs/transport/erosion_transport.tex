...
\begin{abstract} 
  ...
  the fluid equations and to compute the erosion rate, and a stable
  second-order time stepping method to erode the grains.  In our
  original work, we required that the eroding grains were sufficiently
  separated to guarantee that the trapezoid rule provides a sufficient
  amount of accuracy. We now remove this restriction by introducing a
  Barycentric quadrature rule whose error is bounded independent of the
  geometry.  The Barycentric quadrature rule is computationally
  expensive, so it is only used when necessary, and the spectrally
  accurate trapezoid rule is used otherwise. We also simulate and
  analyze transport through eroded porous geometries.  By using the
  Barycentric quadrature rule, the analysis examines flow both in the
  fast and slow regions with high-order accuracy. Our results
  demonstrate the effects of erosion on tortuosity, dispersion, and pore
  size distributions.
\end{abstract}
