...
\otimes \nn(\yy)$, $\xx,\yy \in \Gamma$, which removes the rank one null
space resulting from the flux-free requirement of $\ff$.  In this work,
$\ff$ is the prescribed velocity, $\UU$, on the outer wall, $\Gamma$,
and is zero on the eroding bodies, $\gamma_\ell$, $\ell=1,\ldots,M$.  
...
vorticity of the double-layer potential~\eqref{eqn:stokesDLP} are
\begin{align}
...
\begin{align}
  ...
  \label{eqn:defJump}
\end{align}
must be added to $\ssigma^\DD(\xx)$ for $\xx \in \gamma$.  Finally, the
...
However, so that we can apply a particular quadrature method, we need to
write these layer potentials in terms of Cauchy integrals in $\CC$ and
their derivatives.  The first step is to write the Laplace double-layer potential as the complex integral
\begin{align}
...
To rewrite the Stokes double-layer potential~\eqref{eqn:stokesDLP} in
...
\in \bd\Omega$, except that the jump condition~\eqref{eqn:defJump} is
included.  Finally, to compute the shear stress, the deformation tensor
on $\bd\Omega$ is applied to the normal and tangent vectors as defined
...
consider and have a negligible effect on the tortuosity (for example,
see Figure~\ref{fig:Eroding20tort_all}).  Since
...
definition for the majority of the tortuosity calculations.
...
simulate erosion.   The integral equation is solved with a collocation
method (Section~\ref{sec:spatialDiscretization}).  Depending on the
proximity of the target and source points, either the trapezoid rule or
the Barycentric quadrature rule (Section~\ref{sec:bary}) is applied.  A
selection algorithm that determines which quadrature method is applied
is described in Section~\ref{sec:fmm}.  Once the shear stress is
computed, the bodies are converted to a {\thL}
framework~\cite{hou-low-she1994}.  Then, the erosion model is integrated
forward a single time step.  By using a {\thL} formulation, the stiff
curvature term is linear and the discretization points remain equispaced
in arclength.
...
double-layer representation can all be written in terms of Cauchy
...
quadrature error is arbitrarily large at a fixed resolution.  There have
been a variety of near-singular integration schemes developed.   We opt
to use a method first outlined by Barnett et al.~\cite{bar-wu-vee2015}
which we call the {\em Barycentric quadrature rule}.  Quadrature rules
for only the Cauchy integral and its first derivative are presented, and
we extend this result to include the second derivative.
\todo[inline]{Dump of text}
...
In Section~\ref{sec:transport}, we require trajectories of passive
tracers passing through an eroded geometry.  Given such a geometry
$\Omega$, a tracer $\ss(t)$ initialized at $\ss_0$ is governed by 
\begin{align}
...
the proximity of $\ss(t)$ to $\bd\Omega$, we either use the trapezoid
rule or the Barycentric quadrature rule as described in
Section~\ref{sec:fmm}.
...
[-1,1]$ with rounded off corners.  We initialize the bodies randomly in
$[-1,1] \times [-1,1]$ using a randomized method that chooses a random
center and radii, and then accepts the new body if it is sufficiently
...
are separated by less than 10\% of an arclength spacing.  For all
simulations, we discretize $\Gamma = \bd \Omega$ with $N_\out=1024$
...
\begin{itemize}
  ...
  anomalous dispersion rates, we compute distributions of the pore
  throat sizes and examine their response to erosion.
...
(as p=37.68\%) and another six porosities during the eroding. The dashed line
correspondes to a porosity of 100\% and the dashed-dotted lines are lines
of best fit with slope 1.0644 (as p=95.1\%), 1.0673 (as p=85.09\%), 
1.0594 (as p=75.15\%), 0.9664 (as p=65.09\%), 
0.7766 (as p=55.10\%), 0.7475 (as p=45.08\%), 
and 0.5607 (as p=37.68\%.}
\end{figure}
