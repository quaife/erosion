...
\begin{abstract} 
  ...
  through \nick{a specific class of two-dimensional porous geometries, 
  namely those formed by fluid-mechanical erosion.}  
  We investigate the tortuosity and
  dispersion by analyzing the first two statistical moments of tracer
  trajectories. \nick{For most initial configurations, tortuosity decreases in time 
  as a result of erosion increasing the porosity. 
  However, we find that tortuosity can also increase transiently in certain cases.}
  \nick{The porosity-tortuosity relationships that result from our simulations 
  are compared against models available in the literature.}
  \nick{Asymptotic dispersion rates are also strongly affected by the
  erosion process, as well as by the number and
  distribution of the eroding bodies.}  
  Finally, we analyze the pore size
...
numerical methods \nick{(I killed the reference here because I think its redundant)} to enable simulations of more
...
ris-moo-chi-she-zha2012, moore2013self, par-izu2000}.
\nick{We compute the vorticity in the fluid bulk since, on solid boundaries, vorticity reduces to shear and thus provides a convenient way to simultaneously visualize local erosion rates and changes in the surrounding flow} (figure~\ref{fig:Eroding50vort}).  
A variety of numerical methods for
...
\begin{align}
  \DD[\eeta](\xx) = \frac{1}{2\upi} \int_{\bd\Omega} 
    \frac{\rr \cdot \nn}{\rho^2}\eeta(\yy)\, ds_\yy = \Real (v(x)) \quad 
...
\Real (v'[\psi_2](x)) + \Imag (v'[\psi_1](x)) 
 + \Real (v'[\eta_2](x))+ \Imag (v'[\eta_1](x)), \quad x \in \Omega.
\end{align}
...
work of~\citet{ioa-pap-per1991} (see equation (2.12)).  We start with
...
work~\citep{qua-moo2018}. However, by resolving the interaction between
...
work of~\citet{mat-kha-koz2008} (see figure 4(a)), except that our
...
tortuosity to~\citet{mat-kha-koz2008} (figure 4(b)), the results are
...
%
%{\color{red} We use the Fast Multipole Method (FMM)~\citep{gre-rok1987, gre-gre-may1992} with trapezoid rule to calculate the double-layer potential.
%}
...
%wall.  In~\citep{qua-moo2018} we reported the quick expanding space at
...
%from the one in~\citep{matyka2008tortuosity} since we choose our cross
