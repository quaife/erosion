...
double-layer potential can also be expressed as the sum of a Laplace
double-layer potential 
\begin{align}
  \DD[\eta](\xx) = \frac{1}{2\pi} \int_{\bd\Omega}
    \frac{\rr \cdot \nn}{\rho^2} \eta(\yy) ds_\yy
\end{align}
and its gradient.  By identifying $\xx,\yy \in
\RR^2$ with the points $x,y \in \CC$, the Laplace double-layer potential
for a real-valued density function $\eta$ can also be written as the
real part of the Cauchy integral
\begin{align}
...
Cauchy integrals and its first derivative.  Rather than immediately
applying a quadrature rule to compute $v(x)$, we will first compute the
boundary data of $v(x)$ by applying the Sokhotski-Plemelj jump relation
\begin{align}
  v^{-}(x) = -\frac{1}{2} \eta(x) - \frac{1}{2\pi i} \text{PV}
\end{align}
...
\todo[inline]{BQ: Need to discuss boundary data and how to get it from
density $\eta$}
\begin{align}
  \label{eqn:cauchy}
...
We could find the explanation from the tracer test in Fig.~\ref{fig:Eroding20tracer}. Since the direction of flow is from left to right, most of the tracers will not travel vertically but horizontally. If the trajectories of tracer  }
