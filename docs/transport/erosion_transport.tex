...
{\color{red}
To numerically approach the nearly singular integraton by using Barycentric quadrature rule, 
...
{\color{red} \item Discussion of the choices of $\alpha_1$ and $\alpha_2$ with respect to accuracy and  efficiency

To find the suitable values of $\alpha_1$ and $\alpha_2$ in~\eqref{eqn:Cw2b} and \eqref{eqn:Cb2b}, we set $\alpha_1=\alpha_2=1, 2, 3$, and $4$ to test the criteria on a 20-body problem with $N_\out = 1024$ and $N_\iin = 128, 256$, and $512$. To check the accuracy of the simulations, we use (?)
...
the resolution}
{\color{red} We first consider a circular body in a shear flow. The domain $\Omega$ is a rectangle $[-3,3]\times[-1,1]$ with rounded corners. The velocity of flow form the left side of $\Omega$ is $v(x_1,x_2)=x_2+1$. Let the radius of the body be $r$, the length of the body be $L$, $N_\iin$ and $N_\out$ be the number discretization points on the body and wall respectively, $h=L/N_\iin$ be the resolution mesh size of the body, and $d$ be the initial minimum distance from body to wall. 
