...
bulk.  For $x \in \nick{\CC(\Omega)}$, the Cauchy integral
...
\todo[inline]{This opening sentence is awkward and seems like a run-on
sentence.} Each eroding body is initially a circle with center $\cc_i$
and radius $r_i$.  The center and radius are chosen at random, and the
body is accepted if it is contained in $[-1,1] \times [-1,1]$ and is
...
quadrature method.  They initialized the \st{eroding} body \st{so that}
...
eroded geometry at porosity $\phi = 63.0\%$ \todo[inline]{Make
consistent with caption in figu 13 and another caption somwhere else in
this section}
