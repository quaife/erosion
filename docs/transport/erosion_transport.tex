...
points is required to satisfy the $5h$ threshold \nick{\st{required to guarantee
that} needed for} the trapezoid rule to achieve machine precision.  By applying the
...
number of streamlines that take a nearly direct \nick{\st{pact} path} through the
...
decrease in the dispersion rate. In particular, \nick{once erosion has eroded}
\todo[inline]{Erosion has eroded?}
...
absent, and the average tracer travelled 12\% farther \nick{\st{than it would have
if the bodies were absent,} indicating a tortuosity of $1.12$.}
\todo[inline]{Here and in a previous example, I believe the qualifier `than it would have if the bodies were absent' is obvious from the first part of the sentence and does not need to be repeated because it makes the sentence very verbose.}
...
able to \nick{per}form stable simulations of erosion with $N = \mathcal{O}(100)$
...
eroded geometries \nick{\st{can not} cannot} be completely described in terms of the
...
formulation of the Navier-Stokes equations, \nick{which is an active area of research with a few promising algorithms recently proposed}~\cite{gray2019boundary, kli-ask-kro2019}. 
\todo[inline]{Okay with you if I include my work with Lenny here? After all, the end goal of it is to solve Navier-Stokes with boundary integrals :)}
Regarding other transport models, this would involve a diffusive term to consider the
