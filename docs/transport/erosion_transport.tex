...
ris-moo-chi-she-zha2012, qua-moo2018}.  The incompressible Stokes
equation are solved by first recasting them as a boundary integral
equation (BIE).  The BIE is discretized with a high-order quadrature
...
anomalous diffusion, and the distribution of geometrical channels.  
...
Gap sizes are important for network models ...

The numerical methods to solve the governing fluid and erosion equations
are based on previous work of the authors~\cite{qua-moo2018}.  The fluid
solver uses a second-kind integral equation that is discretized with the
trapezoid rule and solved iteratively with fast multipole accelerated
GMRES, and then the trapezoid rule is used to compute the shear stress
which is required to find the erosion rate.  The integral
equation formulation only requires discretizing the boundary of the
geometry, the complex porous matrix can be easily resolved.  Once the
erosion rate is computed, the bodies are eroded using a
{\thL}~\cite{hou-low-she1994} coordinate system.  This allows us to
maintain a discretization of the eroding bodies that is equally spaced
in arclength, and is guaranteed to not tangle.
