...
%Flow in porous media is further complicated when there is a two-way coupling between the flow and the geometry. For example, the grains of a porous media evolve in time in applications involving
\nick{Flow in porous media is further complicated when boundaries evolve dynamically in response to the fluid flow. This type of coupling between geometry and flow occurs, for example, in applications involving} melting~\cite{bec-vis1998, rycroft2016asymmetric, jambon2018singular, favier2019rayleigh, morrow2019moving}, dissolution~\cite{kan-zha-che-he2002, mac2015shape, moo2017, wykes2018self}, deposition~\cite{joh-eli1995,
...
magnitude~\cite{all-hea-lab-rei2002}.  \nick{Moreover, channelization creates heterogeneous and anisotropic medium properties, which affect the
...
simulations of eroding porous media, and, second, \nick{characterization of tracer transport through the dynamic geometries that result}. Our modeling efforts build on previous work~\cite{ris-moo-chi-she-zha2012, moore2013self, moo2017}, in particular recent numerical methods developed to simulate erosion in the regime of Stokes flow~\cite{qua-moo2018}. We, however, make a few
...
grains (Figures~\ref{fig:Eroding50vel} and~\ref{fig:Eroding50vort}).  To characterize transport through these \nick{configurations}, we examine
...
eroding grains.  In this work, we \nick{\st{do}} introduce a new quadrature method to resolve dense suspensions of eroding grains.  The accuracy of the
trapezoid rule, which we used in our previous work, is \st{sufficient} \nick{adequate} for bodies that are sufficiently separated~\cite{tre-wei2014}. 
\todo[inline]{I changed the above to `adequate' so that `sufficient' would not be used twice in a row.} 
However, it \nick{loses accuracy whenever \st{two} grains come in close contact}.  One of the earliest works \nick{to address} quadrature methods for nearly-singular integrands was developed by Baker and Shelley~\cite{bak-she1986}, and in recent years, many other near-singular integration schemes have been \nick{followed}~\cite{oja-tor2015, kli-tor2018, hel-oja2008a, bea-yin-wil2016, bea-lai2001, klo-bar-gre-one2013}.  We use a Barycentric quadrature
\nick{method}~\cite{bar2014, bar-wu-vee2015} since it is a non-intrusive
modification of the trapezoid rule, and the error is guaranteed to be
uniformly bounded.  We extend the original quadrature method \nick{to compute the velocity gradient, which is needed to evaluate shear stress and fluid vorticity}.
...
To characterize the transport, particle trajectories \nick{must} be
...
this work, we assume trajectories \nick{to be governed by pure advection} (ie.~no diffusion). Therefore, the particle trajectories are
...
\todo[inline]{The logic from the sentence above to the one below seems awkward to me. The above sentence makes me think that a continuous time random-walk will be used in our paper. Then, suddenly, we say we are going to do something else instead. Maybe we can kill the above sentence and dump the references (if you want to keep them) into the earlier sentence that mentions a random walk? Or maybe just a slight rephrasing would make the transition less awkward.}
Instead,  we compute
...
Once \nick{the streamlines are computed, we characterize
transport by analyzing} three different metrics: the tortuosity, the anomalous dispersion, and the pore size distribution. 
The local tortuosity of a streamline \nick{that connects the inlet to the outlet is defined as the streamline's length normalized by the linear inlet-to-outlet distance}.
In porous media, the local tortuosity can be greater than 1.5~\cite{kop-kat-tim1996, mat-kha-koz2008} or even 2~\cite{dud-koz-mat2011}, \nick{depending on several factors such as the porosity.} 
...
dea-leb-den-tar-bol-dav2013}. \nick{Since anomalous dispersion results from streamlines spending time in both the high and low velocity}
regimes~\cite{ber-sch2001}, it is crucial to accurately resolve  streamlines near grain boundaries, as achieved in this work.
Finally, \nick{the distribution of pore sizes is required to} quantify velocity distributions~\cite{ali-par-wei-bre2017, dea-qua-bir-jua2018}, channelization~\cite{sie-ili-pri-riv-gua2019}, connectivity~\cite{knu-car2005, wes-blo-gra2001}, and to develop network models~\cite{bry-kin-mel1993, bry-mel-cad1993, bij-blu2006}. \nick{Hence, we construct the pore-size distribution throughout the erosion process.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Contributions}
Our first contribution is extending the Barycentric quadrature rule of
Barnett et al.~\cite{bar-wu-vee2015} \nick{to accurately compute velocity gradients associated with Stokes flow.}
\nick{This new quadrature rule permits accurate and stable simulations of erosion, and, moreover, enables visualization of the vorticity field throughout.}
...
\todo[inline]{The above paragraph seems to repeat a lot of what has been mentioned already, but maybe that is okay.}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\paragraph{Limitations}
As in our previous work~\cite{qua-moo2018}, we consider two-dimensional
immobile eroding grains. However, this work \nick{does not limit} the proximity of  grains. Compared to our previous work, we are able to
...
In Section~\ref{sec:formulation}, we summarize the erosion model \nick{that is described in more detail in} previous work~\cite{qua-moo2018}.  In
...
method \nick{for computing} shear stress and vorticity.
...
\nick{briefly summarize the model, since a more detailed description can be found in previous} work~\cite{qua-moo2018}.  We consider flows inside a confined geometry $\Omega$ that contains $M$ eroding bodies with boundaries $\gamma_\ell$, $\ell = 1,\ldots,M$.  
\nick{The boundary of the fluid domain is $\bd \Omega = \Gamma \cup
\gamma_1 \cup \cdots \cup \gamma_M$, where $\Gamma$ is the outer boundary, taken to be a slightly smoothed version of $[-3,3] \times [-1,1]$}. 
\nick{We only place eroding bodies in $[-1,1] \times [-1,1]$ so as to create a buffer region for the flow profile imposed at the inlet to transition to the more complex flow intervening between the bodies}. On each
...
each time step \nick{to achieve} a constant pressure drop across the
...
=======
\nick{To solve} the incompressible Stokes
equations in complex two-dimensional porous media, we \nick{reformulate\st{d}} the incompressible Stokes equations as a BIE.  This has the advantage that
only the one-dimensional boundary of the domain must be discretized, and,
\nick{with an appropriate quadrature formula and fast summation methods,
the result is a high-fidelity numerical simulation of near-optimal computational complexity.}
>>>>>>> 98d3b24aecce2b71c6cf058a17884c96953ef55a
...
\end{align}
where $D$ is \nick{\st{an} a} kernel of the integral operator, $\rr = \xx - \yy$,
...
$N_0(\xx,\yy) = \nn(\xx) \otimes \nn(\yy)$, $\xx,\yy \in \Gamma$,
which removes the rank one null space \nick{\st{resulting} that results} from the flux-free
...
Stokeslets $\llambda_\ell$, and the rotlets $\xi_\ell$, \nick{we are able to compute the deformation tensor, pressure, and vorticity, defined respectively as}
\begin{align}
...
=======
\nick{\st{We require the deformation tensor of the double-layer potential at
$\xx \in \bd \Omega$.}}
Since the deformation tensor of the double-layer potential is discontinuous across the boundary $\bd\Omega$, the jump term 
>>>>>>> 98d3b24aecce2b71c6cf058a17884c96953ef55a
\begin{align}
...
Stokeslets and rotlets \nick{do not exhibit such jumps, and so the associated components of the deformation tensor, pressure, and vorticity} are straightforward to compute~\cite{poz1992}.  Having computed
