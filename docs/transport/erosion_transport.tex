...
\begin{align}
  \left\|\cc_i - \left[
    ...
    \end{array}
    \right]
  \right\| < L_i + \alpha_\out \frac{L_\out}{N_\out}.
\end{align}
...
find that $\alpha_\iin = ???$ and $\alpha_\out = ???$ give sufficient
...
the eroding bodies, we apply smoothing the smoothing parameters
$\eps=???$ and $\sigma=???$, as described in~\cite{qua-moo2018}, for all
simulations.  

Finally, the parameters used to determine if a point uses the trapezoid
or Barycentric quadrature rule are $\alpha_\iin = ???$ and
$\alpha_\out=???$.

\todo[inline]{time step size}
