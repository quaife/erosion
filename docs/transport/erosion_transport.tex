...
\end{itemize}
{\color{red} In this section, we present the numerical simulations of bodies eroding in the fluid. The computational domain $\Omega$ is a rectangle $[-3,3]\times[-1,1]$ with rounded off corners and we place bodies only in $[-1,1]\times[-1,1]$. There are $N_\out = 1024$ points on the outer boundary for all the simultions. We impose the fixed pressure drop condition. We use smoothing parameters of $\eps$ and $\sigma$ to control eroding motion on the bodies.  
}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
the initial length of the body be $L$, $N_\iin$ be the number of discretization points 
on the body, 
