...
\todo[inline]{The above set of references for ``examples of course-grained models" seems very thorough and perhaps not as relevant as other references on erosion, melting, etc. JFM's reference format makes them take up a lot of space. So if there are references here that you wouldn't mind dropping, it might help. I wouldn't drop all of them. I think at least 3-4 are definitely needed.}
...
industrial applications~\citep{ris-moo-chi-she-zha2012, berhanu2012shape, hewett2017evolution,
...
knu-car2005}, and cellular automata~\citep{rot1988}.  
\todo[inline]{The set of references above seems to me like an unneeded `straw-man' argument. i.e.~Just because we are using a boundary integral method, does that mean we have to list every other possible way of solving the fluid equations? It takes up a fair amount of space. On the other hand, if there are references here you want to keep, by all means do it.}
However, simulating erosion requires a method that is high-order, can resolve
...
To compute stable simulations of erosion, we use 
\nick{methods of high order in both space and time.}
The time integration is unchanged from
...
but not for grains in close contact.  
One of the earliest quadrature methods for nearly-singular
integrands was developed by~\citet{bak-she1986}, and in recent years,
many other \nick{\st{near-singular integration}} schemes have
...
\begin{align}
  ...
  \label{eqn:tracers}
\end{align}
\todo[inline]{Could the above equation be changed to inline to save space?}
...
Runge-Kutta time stepping method. The Barycentric
...
\nick{motivated by the geological situation of a porous medium connecting two regions of  fixed hydraulic heads.}
...
\todo[inline]{The above vertical fraction could be made into a horizontal fraction, which would save a lot of space.}
...
\todo[inline]{More prime examples of vertical fractions that could be made horizontal.}
...
(section~\ref{sec:transport}), we require accurate computation of the streamlines
governed by equation~\eqref{eqn:tracers}.
\nick{\st{To characterize the transport, it is crucial that the streamlines are computed accurately.}}
...
rendering the trajectory meaningless. \nick{\st{to quantify transport.}}
\nick{\st{However, if we simply ignore trajectories that pass close to grains, this would
significantly skew the characterization of the transport. } 
However, simply ignoring trajectories that pass close to a grain could significantly bias the characterization of transport.}
Therefore, we use high-order quadrature and time stepping methods.  In particular,
...
Stokes flow and analyze transport through the evolving
geometries. \nick{\st{by examining coarse-grained variables}}.  
Each \nick{\st{eroding}} body is
initialized as a circle of center $\cc_i$, radius $r_i$, and length $L_i = 2\upi r_i$.  
\todo[inline]{There were too many sequential uses of the word `eroding' above, so I cut two of them out.}
The center and radius are chosen at random, and the body is
accepted if it is contained in $[-1,1] \times [-1,1]$ and is
sufficiently separated from all other bodies.  
\awk{Since the error of our quadrature method is bounded independent of the geometry, we consider bodies that are separated by less than 10\% of an arclength spacing.}
\todo[inline]{The above sentence seemed like a non sequitur to me. See if I got the right idea across in the sentence below.}
\nick{Owing to our adaptive quadrature rule, we can consider bodies that are separated by less than 10\% of an arclength spacing.}
...
\todo[inline]{I was proud of myself for catching the missing period at the end, so had to point it out :)}
...
vanished.
\todo[inline]{accurately resolved to the desired accuracy is redundant.}
...
\todo[inline]{Perhaps equations like the above could be made inline to save space.}
...
\end{align}
\todo[inline]{This one definitely seems like a candidate for an inline equation.}
...
\todo[inline]{Very long captions in Figs. 10 and 12 and maybe others.}
...
\end{align}
\todo[inline]{More inline candidates}
...
we observe that for certain \nick{\st{porous media} configurations}, the tortuosity \nick{transiently increases, even though the porosity always increases due to erosion.}
\nick{\st{as erosion increases the porosity of the porous media.}} 
We also observe super-dispersive spreading, and the rate of dispersion significantly
...
provide \nick{\st{preliminary}} results for the pore throat sizes which affect the
