...
be as small as $10^{-1}\mathrm{m/day}$~\cite{kut-scr-dav-ham1995}, \awk{and
the scales of a single porous media can range over several orders of
magnitude.}
%
\todo[inline]{Nick: I am confused by the last part of the sentence (highlighted in purple). What `scale' are you referring to? Length? You already described the length scales in the first part of the sentence. Maybe you mean to distinguish between the grain length scales and the overall length scales of the system. If so, you can use adjectives like `grain' and `macroscopic' or `overall' to clarify that.} 
%
Numerical methods that \st{accurately address all of these} \nick{resolve this range of} scales offer the ability to: (i) characterize dispersion~\cite{saf1959}, (ii) quantify mixing~\cite{leb-den-dav-bol-car-dec-bou2011, den-leb-eng-bij2011}, and (iii) potentially develop meaningful constitutive relationships that link the microscopic and macroscopic realms~\cite{mil-chr-imh-mcb-ped1998}. 
%
\todo[inline]{Nick: I reworded the above sentence. See if you agree with my changes.}
%
...
porous media evolve in time in applications involving melting~\cite{bec-vis1998, rycroft2016asymmetric, favier2019rayleigh, morrow2019moving}, dissolution~\cite{kan-zha-che-he2002, mac2015shape, moore2017riemann, wykes2018self}, deposition~\cite{joh-eli1995, hewett2018modelling}, biofilm growth~\cite{tan-val-wer2015}, and \nick{crack formation} \cite{cho2019crack}.  
%
\todo[inline]{Nick: I added some references. Check out the last one on crack formation in shrinkable, granular packings. It is really cool work done by an excellent experimentalist Sujit Datta. I met him and he voiced a strong interest in our work, so wanted to make you aware.}
%
We focus on erosion, a fluid-mechanical
process that is prevalent in many geophysical, hydrological, \nick{and industrial} 
applications \cite{berhanu2012shape, hewett2017evolution, lopez2018cfd, allen2019sde, amin2019role}.
\todo[inline]{Nick: Check out the first reference, Berhanu, Kudrolli, Rothman PRE 2012. Maybe we can cite it a couple more times throughout since it has both the words `erosion' and `granular bed' in the title :)}
When a porous geometry erodes, certain qualitative characteristics are unveiled that affect transport through the geometry.
For example, an eroded geometry may contain channels of high porosity, \nick{which, though few in number and modest in volume fraction, transmit a large portion of the flux}. This arrangement results in velocities
that vary over several orders of magnitude~\cite{all-hea-lab-rei2002}.
%
\todo[inline]{Nick: I rearranged the above two sentences. See if you like it.}
%
Moreover, \nick{channelization creates a heterogeneous and anisotropic medium properties}, \st{and
this affects} \nick{affecting} the transport of \st{quantities} \nick{tracers} such as
contaminants~\cite{cve-che-wen1996, dag1987, kon-bre1978} and
heat~\cite{nil-sto1990, ree-sto1995}.  \awk{To characterize transport in
...
geometries}.  
\todo[inline]{Nick: The purple sentence is redundant: `To \underline{characterize transport} ... we undertake ... \underline{characterizing transport} through these geometries. Maybe try something like this:}
\nick{This study consists of two main undertakings: First, high-fidelity simulations of porous-medium erosion, and, second, the characterization of transport through the resulting geometries.}
Our modeling efforts build on previous work \cite{ris-moo-chi-she-zha2012, moore2013self, moore2017riemann}, in particular recent numerical methods developed to simulate erosion in the regime of Stokes flow \cite{qua-moo2018}. We, however, make a few key improvements to the numerical methods from \cite{qua-moo2018} to enable simulations of more realistic, dense suspensions of eroding grains (Figures~\ref{fig:Eroding50vel} and~\ref{fig:Eroding50vort}).  To characterize transport in the resulting geometries, \st{we apply statistical methods to develop coarse-grained variables} \nick{examine coarse-grained variables through statistical analysis of tracer trajectories}.
\todo[inline]{I'm not sure about my modification above, but see if it gets across the message that was intended.}
...
\st{Because of the length and velocity scales in hydrological applications
such as groundwater flow} \nick{Owing to the scales present in groundwater flow} \cite{bea1972}, we model the hydrodynamics with
the two-dimensional incompressible Stokes equations.  \st{To couple erosion
with the fluid} Meanwhile, individual grains erode at a rate \st{that is} proportional to \st{the magnitude of} the hydrodynamic shear stress~\cite{wan-fel2004,
ris-moo-chi-she-zha2012, moore2013self, par-izu2000}.  Since the vorticity and the shear stress are equivalent on the grains in the zero Reynolds number
limit, we also compute the vorticity in the fluid bulk \nick{for visualization purposes} (Figure~\ref{fig:Eroding50vort}).  A variety of numerical methods for
...
be a slightly rounded-off version of $[-3,3] \times [-1,1]$, and we only
...
\label{sec:DLP}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Since we require a method to solve the incompressible Stokes equations in
complex two-dimensional porous geometries, we reformulated the
incompressible Stokes equations as a boundary integral equation. This
has the advantage that only the one-dimensional boundary of the domain
must be discretized, and if appropriate quadrature formula and fast
summation methods are used, the result is a high-fidelity numerical
simulation that is computed with near-optimal complexity.
...
the grain configuration is important for pore network
...
using nodes placed at the center of each eroding grain and at a collection
...
neighbors. 
{\color{red} Since Delaunary triangulation is not unique, we only regenerate 
a new triangulation after a body has been eliminated.}
...
discretization~\cite{hou-low-she1994}.  The time stepping methods for
...
as the fast multipole method~\cite{gre-rok1987}, this cost can be
...
points that are too close to $\bd\Omega$.  Note, however, that since a
point is typically only close to one or two components of $\bd\Omega$,
only the contribution of these nearby bodies needs to be replaced.
Assuming that $\xx$ is too close to $\gamma_k$, we first subtract the
inaccurate trapezoid rule approximation of the double-layer potential
due to $\gamma_k$.  Then, the Barycentric quadrature rule is used to
compute the velocity due to $\gamma_k$ with more accuracy.  That is, the
final velocity is
\begin{align}
...
smoothing parameters $\eps=15/256$ and $\sigma=10/256$.
...
\begin{align}
  ...
  \right],
\end{align}
imposed on the outer wall. Mitchell and Spagnolie~\mit{spa-spa2017} did
...
discretization points.  For all three simulations, we use the time step
size $\Delta t=10^{-4}$.
...
anomalous dispersion rate of eroding geometries.  To achieve statistical
...
Figure~\ref{fig:Eroding20tort}(a) for the eroded geometry with porosity
62.9\% (see Figure~\ref{fig:Eroding20tort}(c)).  The velocities are
...
Hagen-Poiseuille flow. The 4 snapshots are evenly spaced in time, and
...
(a) The $x$-component of the velocity at the inlet, $u_1(-1, y)$,
normalized by its maximum velocity $u_{max}=3.90 \times 10^{-4}$.  (b)
The local tortuosity $\tau(y)$ on the cross section $x = -1$. The
...
horizontal flow, $u_1(y)$ increases since there is no longer a no-slip
