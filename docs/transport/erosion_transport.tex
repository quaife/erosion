...
efforts build on previous work~\cite{ris-moo-chi-she-zha2012,
moore2013self, moo2017}, in particular recent numerical methods
developed to simulate erosion in the Stokes-flow
regime~\cite{qua-moo2018}. We, however, make key improvements to the
numerical methods from~\cite{qua-moo2018} to enable simulations of more
...
vorticity and the shear stress are equivalent on \nick{solid boundaries}, we also compute the fluid vorticity for visualization purposes (Figure~\ref{fig:Eroding50vort}).  A variety of numerical methods for the fluid solver are available including
...
To characterize transport \nick{through the resulting configurations}, particle trajectories must be computed. Depending on the application, microscale transport can be modelled as
pure advection~\cite{dea-qua-bir-jua2018, leb-ded-dav-bou2007,
...
between solving the fluid equations \nick{and} advancing the \nick{erodible boundaries}. The strength of $\UU$ is adjusted at each time step to achieve a constant pressure drop across
the channel.
...
is an unknown density function.  The double-layer potential \nick{cannot}
...
addressed with \nick{\st{the integral operator}} $\NN_0$ which is the integral
operator having kernel $N_0(\xx,\yy) = \nn(\xx) \otimes \nn(\yy)$,
$\xx,\yy \in \Gamma$.  In this work, $\ff$ is the prescribed velocity, \nick{which is equal to  $\UU$ on the outer wall, $\Gamma$, and equal to zero on the eroding bodies,
...
\begin{align}
  \DD[\eeta](\xx) = \frac{1}{2\pi} \int_{\bd\Omega} 
    ...
    \frac{\eta(y)}{x - y} \, dy, \quad x \in \nick{ \CC(\Omega) },
  \label{eqn:laplaceComplex}
\end{align}
where $x,y \in \CC$ are the complex counterparts of $\xx,\yy \in \RR^2$,
and $\eta$ the complex counterpart of $\eeta$. \nick{We use the notation $\CC(\Omega)$ to denote the physical domain $\Omega$ represented in complex coordinates.}
\todo[inline]{What do you think of this notation I introduced to distinguish between $\Omega$ as a real versus complex domain? I was skimming over this section and was very confused about what $x$ and $y$ were, having naively assumed they were physical $x$ and $y$ coordinates instead of complex numbers. Hence I wanted to make their definitions more prominent. I realize that you state $x$ and $y$ are complex numbers, but it is in the text and could be missed by the careless reader (i.e. me). However, I am not married to this notation and could easily be convinced just to leave it as you had it.}
Equation~\eqref{eqn:laplaceComplex} is converted to a Cauchy integral by
first finding the boundary data of $v$.  Assuming that $\nick{\CC(\Omega)}$ is a
...
\begin{align}
  ...
  v''(x) = \frac{1}{\pi i} \int_{\bd\Omega}
    \frac{v(y)}{(y-x)^3} \, dy,
\end{align}
for $x \in \nick{\CC(\Omega)}$.  Since $v(x)$ depends on the complex-valued density
...
potential representation of the \nick{velocity gradient}.  The
deformation tensor at $x \in \Omega$ is found by computing the
derivatives of the expressions for $u_1$ and $u_2$ in
...
the normal and tangent vectors as in
equation~\eqref{eqn:shearStress}.  The \nick{velocity gradient} is
also used to compute the vorticity in the fluid bulk.  For $x \in
\nick{\CC(\Omega)}$, the Cauchy integral representation of the vorticity is
\begin{align}
...
particle reaches the \nick{outlet} of the porous region, it is reinserted at the \nick{inlet}.  To minimize errors caused by reinsertion, the particle is initialized at the discretization point that has the closest velocity to the particle's velocity at the outlet.  After a
...
options.  
\todo[inline]{What does it mean `how streamlines are defined'? I had thought that the definition of a streamline is unambiguous. Do you mean which streamlines are used? i.e.~If there is a bias towards either shorter or longer streamlines?}
\nick{\st{One choice, that we adopt, uses }} The choice adopted here is to use
streamlines that originate at $S$ and terminate at a parallel cross-section.  We use the
...
\todo[inline]{This figure seemed out of place to me because you do not present other graphs yet, only the distribution of pores, and it is before the numerical methods are explained. HOWEVER, I think I see what you are driving towards: I think you want to give the reader a little teaser of the results, so that they are more inclined to make it through the methods section without losing interest (is that more or less right?). So perhaps just a few words to indicate our intent would prevent the reader from getting whiplash; maybe phrases like ``As illustration of the statistical results that will be elaborated in Section blah, we show here....''
}
...
{\thL} formulation.  In addition, we apply regularization~\cite{qua-moo2018} to \nick{\st{avoid} slightly smooth} the corners that inevitably develop during erosion.
\todo[inline]{Note: We do not want to say that we avoid corners because that makes it sound like our numerical simulation fails to capture the real physical phenomenon of corner formation.}
...
\label{sec:bary}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\st{Since we are applying quadrature to periodic smooth functions, the
trapezoid rule is spectrally accurate. However, since
...
\nick{While the trapezoid rule is spectrally accurate for smooth, periodic functions~\cite{tre-wei2014}, the derivative of the integrands grows as the target point $\xx$ approaches $\bd\Omega$ and so the error bound depends on the location of the target point.} Therefore, if the trapezoid rule is applied when bodies are in near-contact, or when a layer potential is evaluated at a point close to $\bd\Omega$, then the results become unreliable and the \nick{simulation} ultimately becomes unstable.  We thus \nick{desire a quadrature method whose error bound does not depend on the target location}.
...
\label{sec:results}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\nick{In this section, we apply our new quadrature method to simulate
erosion of several different configurations of nearly-touching eroding
bodies, and to characterize transport inside eroded geometries.}
\todo[inline]{This opening sentence is awkward and seems like a run-on sentence.}
Each eroding body is initially a circle with center $\cc_i$ and radius $r_i$.
The center and radius are chosen at random, and the body is accepted \nick{if} it is contained in $[-1,1] \times [-1,1]$ and is sufficiently
...
The erosion rate \nick{\st{has a loss of regularity} loses regularity at stagnation points, which inevitably leads to corner formation on the bodies.} 
As described in Section~\ref{sec:formulation} and our
previous work~\cite{qua-moo2018}, we introduce a curvature penalization
term and a Gaussian smoothing step in order to ameliorate corner formation.  
For all examples, we use the smoothing parameters $\eps=15/256$ and $\sigma=10/256$, and the time
...
\begin{itemize}
  ...
  centered at $y=-1$ \nick{and} compare the eroding body's shape to a similar
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\nick{\st{We} First,} consider a single eroding body close to a solid wall with the
shear flow
\begin{align}
  \UU(\xx)=\left[
  \begin{array}{c}
  ...
  \right],
\end{align}
imposed on the outer wall. Mitchell and Spagnolie~\cite{mit-spa2017} \nick{\st{did} performed} a similar three-dimensional experiment using a second-order quadrature
method. \st{Here} They initialized the \st{eroding} body \st{so that} with its center located $1.5$ \nick{radii} above the solid wall.  In two dimensions, the
...
Figure~\ref{fig:Eroding20vort}.  The color is the vorticity, and the
erosion rate is proportional to the vorticity.  Initially, several of
...
precision.  In particular, the distance between bodies 1, 6, 13, and 15
and the outer wall is $1.3h$, $2.9h$, $2.8h$, and $1.3h$, respectively,
where $h$ is the arclength spacing of the outer wall $\Gamma$.  In
addition, the distance between several pairs of eroding bodies,
including 1 \& 6, 3 \& 9, 6 \& 8, and 14\& 18, is too small to be
accurately resolved with the trapezoid rule.  By using the Barycentric
...
bodies, each discretized with $N_\iin=256$ points, in a Hagen-Poiseuille
flow.  The four snapshots are evenly spaced in time, and the color is
the fluid vorticity. In the fourth frame, bodies 4, 12, and 17 have
vanished completely, and body 19 has almost completely eroded.}
\end{center}
\end{figure}
...
regions of near contact.  This qualitative behavior is present in
Figure~\ref{fig:Eroding20vort} between bodies 3 \& 4, 15 \& 16, and
others.  However, by resolving the interaction between bodies that are
...
common feature of the pores that grow slowly is that they are nearly
...
bodies, each discretized with $N_\iin = 256$ points, in a
Hagen-Poiseuille flow. The four snapshots are evenly spaced in time, and
the color is the fluid vorticity. In addition to the channels that
develop between the bodies and the solid walls, erosion leads to two
main channels through the geometry---one in the top half and one near
the middle.}
\end{figure}
