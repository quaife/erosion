...
quadrature method.  They initialized the \nick{\st{eroding} body \st{so that}
with its center located} $1.5$ radii above the solid wall.  In two
dimensions, the trapezoid rule is spectrally accurate, and an analogous
initial configuration is possible. However, unless the number of
discretization points is increased, the trapezoid rule becomes obsolete
if the eroding body is initialized closer to the solid wall. Therefore,
to simulate erosion near a solid wall at reasonable levels of
discretization, we use our hybrid method that combines the trapezoid
rule and the Barycentric quadrature rule.
...
Figure~\ref{fig:Eroding20vort}.  The color is the vorticity, \nick{which is proportional to the erosion rate on solid boundaries}.
Initially, several of the eroding bodies are closer to the outer wall than the $5h$ threshold \nick{required \st{to guarantee that} for} the trapezoid rule to achieve machine
...
can be accurately resolved \nick{\st{at the prescribed resolution} to the desired accuracy}, and erosion can
...
\nick{\st{Similar to our previous work,}} Erosion causes the space between nearly
touching bodies to quickly expand, and flat faces develop along the
regions of near contact.  This qualitative behavior is seen in Figure~\ref{fig:Eroding20vort} between bodies 3 \& 4, 15 \& 16, and
\nick{was also witnessed in previous work}.  
However, by resolving the interaction between bodies that are
much closer together than in previous work, we observe that very little erosion occurs between certain pairs of bodies, at least initially.  
For instance the opening between bodies 1 \& 6, 3 \& 9, and 5
...
Figure~\ref{fig:Eroding20flowrate}(b) \nick{is roughly linear which indicates
...
\todo[inline]{WAIT! Fig 7b is a semi-log plot with the x-axis linear and the y-axis logarithmic. Therefore, a straight line indicates exponential increase, not a power law.}
...
concentrated, \nick{\st{and this}} corresponding to the regions of highest velocity.  Two of these regions are located between the bodies and the solid walls at
$y=\pm 1$, and the third cuts through the porous region with the upper
part of the channel formed by bodies 1, 4, 6, and 8.  
\todo[inline]{Right now, this description has the reader looking at Fig. 8, but the bodies are not numbered in that figure, so they don't know which ones are 1, 4, 6, 8 without flipping back and forth with the previous figures to try to match corresponding bodes.}
Since the flow is fastest in these regions, the shearing is largest, and this causes the
channels to continue to open fastest \nick{\st{which can be} as} observed in
...
dispersion rate\nick{s} of eroding geometries.  To achieve statistical
...
the grains were absent.  The average streamline is 9.79\% longer
\nick{\st{than if the grains had been absent}, \st{so the} or equivalently the tortuosity of the \st{eroded channel} geometry is $1.098$}.  Again, comparing the local tortuosity to
...
\nick{Discontinuities in local tortuosity \st{happens} occur} when nearby
streamlines diverge \nick{\st{on opposite sides of a grain} to circumvent a grain}.  In
Figure~\ref{fig:Eroding20tort}(c), we plot pairs of streamlines \nick{associate with the ten largest jumps in the local tortuosity, with each pair of corresponding streamlines plotted in the same color.}
...
models~\eqref{eqn:tortuosityModels} and \nick{found the power law to give} the
...
\todo[inline]{I wonder if this initial increase in the tortuosity is specific to the process of erosion, or if it would also occur if we simply took a collection of circular bodies and changed the porosity by controlling the radii? We don't have to answer this question now, but it would be interesting to consider in the future.}
...
computed \nick{\st{at} for} seven geometries of different porosities that are formed
...
power \st{low} law} regimes.  Initially, the dispersion is ballistic
...
dispersion slows \nick{\st{down}}, and we observe super-dispersive ($\sigma_\lambda
...
\todo[inline]{Nick made it to here.}
