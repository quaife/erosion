...
\begin{abstract} 
  ...
  through specific two-dimensional porous geometries---those that have
  become channelized by the process of fluid-mechanical erosion.  We
  consider passive tracers in the eroded geometries and analyze their to
  statistical moments to investigate the effect of erosion on the
  tortuosity and dispersion rates. We compare the computed
  porosity-tortuosity relationship with models available in the
  literature. Finally, we analyze the pore size distribution of an
  eroding geometry. The simulations are performed by combining a
  high-fidelity boundary integral equation solver for the Stokes
  equations, a second-order stable time stepping method to simulate
  erosion, a new numerical method to stably and accurately compute the
  eroded geometries and particle trajectories.
...
microscopic and macroscopic realms~\cite{mil-chr-imh-mcb-ped1998}.
...
distributions~\cite{ica-den2019, bel-sal-rin1992}, pore and flow
distributions~\cite{ali-par-wei-bre2017}, geometry
connectivity~\cite{knu-car2005, wes-blo-gra2001}, anomalous
...
flux \cite{qua-moo2018}. This arrangement results in velocities that
vary over several orders of magnitude~\cite{all-hea-lab-rei2002}.
...
formulation by adding the $M$ Stokeslets and rotlets~\cite{pow-mir1987}
\begin{align}
...
are straightforward to compute~\cite{poz1992}.  Having computed the
...
Cauchy integrals and its first derivative~\cite{bar-wu-vee2015}
\begin{equation}
  \begin{aligned}
...
computing the tortuosity are sometime used~\cite{dud-koz-mat2011}.  The
...
$\phi$.  For example, Matyka et al.~\cite{mat-kha-koz2008} propose the
models
\begin{subequations}
...
velocity scales~\cite{ali-par-wei-bre2017}, correlation
structures~\cite{leb-ded-dav-bou2007}, contaminant
transport~\cite{knu-car2005},
channelization~\cite{sie-ili-pri-riv-gua2019,berhanu2012shape}, and pore
network models~\cite{bry-kin-mel1993, bry-mel-cad1993, bij-blu2006}. To
...
share an edge of the triangulation~\cite{dea-qua-bir-jua2018}. The pores
...
sizes~\cite{ioa-cha1993}.  In Section~\ref{sec:Eroding100}, we
...
discretization~\cite{hou-low-she1994, moore2013self}.  The time stepping
...
functions~\cite{tre-wei2014}, the derivative of the integrand grows as
...
derivatives~\eqref{eqn:cauchy}. Ioakimidis et al.~\cite{ioa-pap-per1991}
...
al.~\cite{bar-wu-vee2015} used these quadrature rules to compute the
...
multipole method (FMM)~\cite{gre-rok1987}, this cost can be reduced to
...
\begin{itemize}
  ...
  experiment of Mitchell and Spagnolie~\cite{mit-spa2017}.
...
recirculation is possible in two dimensions~\cite{chw-wu1975, hig1985}.
...
Erosion causes the space between nearly touching bodies to quickly
expand, and flat faces develop along the regions of near contact.  This
qualitative behavior is seen in Figure~\ref{fig:Eroding20vort} between
bodies 3 \& 4, 15 \& 16, and was also observed in previous
work~\cite{qua-moo2018}. However, by resolving the interaction between
bodies that are much closer together than in previous work, we observe
that very little erosion occurs between certain pairs of bodies, at
least initially.  For instance the opening between bodies 1 \& 6, 3 \&
9, and 5 \& 13 grow much slower than the opening between bodies 15 \&
16.  A common feature of the pores that grow slowly is that they are
nearly perpendicular to the main flow direction, resulting in a small
erosion rate.
...
area fraction resembles our previous work~\cite{qua-moo2018} (Figure
...
Next, we use the $N_p = 1000$~\cite{bel-sal-rin1992} streamlines to
...
tortuosity to Matyka~\cite{mat-kha-koz2008} (Figure 4(b)), the results
...
works~\cite{mat-kha-koz2008}. However, the tortuosity of the eroded
...
proposed~\cite{gray2019boundary, kli-ask-kro2019}.  Regarding other
