...
\end{align}
is the shear stress on the eroding bodies.  Using the same approach as
...
velocity on the outer wall and eroding bodies, and $\NN_0$ is the
integral operator with kernel $\nn(\xx) \otimes \nn(\yy)$, $\xx,\yy \in
\Gamma$, which removes the rank one null space resulting from the
flux-free requirement of $\ff$. Once the unknown density function is
computed, the shear stress is found by first computing the deformation
tensors of the double-layer potential, Stokeslets, and rotlets, and then
forming the shear stress as defined in equation~\eqref{eqn:shearStress}.
...
\label{s:DLPcomplex}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The layer potential representations of $\uu$~\eqref{eqn:stokesDLP} and
its gradient are written in terms of contour integrals in $\RR^2$. We
will be applying a quadrature method for holomorphic functions, so we
must convert these layer potentials in $\RR^2$ to contour integrals in
$\CC$.  The first step is to write the Stokes double-layer potential as
the sum of a Laplacian double-layer potential and its gradient
\begin{equation}
...
integral by first finding boundary data of $v$ which satisfies the
...
\end{align}
\todo[inline]{Need to be careful about interior and exterior limits}
Once the boundary data of $v$ is formed, we have
\begin{align}
  \label{eqn:cauchy}
  v(x) = \frac{1}{2\pi i}\int_{\bd\Omega} 
...
introduced this notation, the components of the Stokes double-layer
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Cauchy Integral Representation for the Gradient of
the Velocity}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We require complex-valued representations of both the shear stress and
the vorticity.  Both these quantities depend on the gradient of the
velocity.  Each term of the gradient of the velocity field can be
written in terms of Cauchy integrals.  Taking the derivatives of
equation~\eqref{eqn:cauchy1} with respect to $x_1$ and $x_2$, we have 
\begin{align}
...
\end{align}
With these four expressions, the deformation tensor of the
double-layer potential, $\ssigma^D = \frac{1}{2} (\nabla \uu + \nabla
\uu^T)$, is written in terms of Cauchy integrals.  In addition, the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The dynamics of fluid in porous media can be characterized in terms of
anomalous diffusion.  However, the anomalous diffusion rate depends on
the properties of the geometry.  In addition to the porosity, the
distribution of the grains affects the anomalous diffusion rates.  To
quantify this rate, we require tracer trajectories that are governed by
\begin{align}
  \label{eqn:trajectories}
  \frac{d\ss}{dt} = \uu(\ss,t), \quad \ss(0) = \ss_0.
\end{align}
Given a trajectory, we define $\lambda_j(t)$ to be the distance
...
\end{align}
If the particle spreading scales with time as $\sigma_\lambda^2 \sim
t^\alpha$, with $\alpha < 1$, this indicates that the particle
trajectories are sub-diffusive.  In contrast, if $\sigma_\lambda^2 \sim
t^2$, the particle trajectories are undergoing a ballistic motion and
are not affected by the grains.

For early times when the geometry has not eroded, we expect the
trajectories to be sub-diffusive.  However, as the bodies erode and
straight preferential channels are formed, we expect the dynamics to
resemble a ballistic motion.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Tortuosity}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The tortuosity of a geometry quantifies the average amount of turning
made by a streamline.  Mathematically, it is defined as
\begin{align}
...
the velocity at the $y$-coordinate along $S$.  Following~\cite{}, the
trajectories are formed until they cross a second cross-section.  In our
work, we always initialize the trajectories at $x=-1$ and stop them when
they reach $x=1$.
...
only constitute a small fraction of the total porous media.  Therefore,
the tortuosity defined in equation~\eqref{eqn:tortuosity2} is an upper
bound for the definition in equation~\eqref{eqn:tortuosity1}.  We have
used formulas for a particular geometry, and the difference is
negligible.  Since equation~\eqref{eqn:tortuosity2} does not require the
additional work of computing particle trajectories, we use this
definition for the tortuosity.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The Barycentric quadrature rule developed by Barnett et
al.~\cite{bar-wu-vee2015} approximates the double-layer potential for
the velocity~\eqref{eqn:stokesDLP} with bounded error.  However, to
compute the shear stress and vorticity, the method must be extended.  We
first briefly describe the original Barycentric quadrature method, and
then derive a necessary extension to simulate erosion.  We present the
quadrature for a simply-connected domain $\Omega$, and the method
naturally extends to the eroding geometry by applying the quadrature to
each of its connected component.  The method starts with an underlying
quadrature rule, and we use the trapezoid rule because of its superior
accuracy for smooth periodic functions~\cite{tre-wei2014}.  Since the
...
We next identify all the discretization points, the normal vector, and
the density function with their complex-value counterparts.  Then, all
the layer potentials are a combination of a Cauchy integral and its
first two derivatives.  Given a density function $\tau(y)$, the boundary
data is solved using the Sokhotski-Plemelj jump
relation~\eqref{eqn:SPrelation}.  If $x \in \Omega$ is too close to
$\bd\Omega$ and the trapezoid rule is applied to the identity
\begin{align}
  v(x) = \int_{\bd\Omega} \frac{v^{-}(y)}{y-x} \, dy, 
  \quad x \in \Omega,
\end{align}
the quadrature error is large since the integrand has a large
derivative.  However, if the trapezoid rule is applied to
\begin{align}
  \int_{\bd\Omega} \frac{v^{-}(y) - v(x)}{y - x} \,dy = 0,
\end{align}
the quadrature error is bounded since the integrand and all its
derivatives are bounded with respect to $x$. That is,
\begin{align}
...
\begin{align}
  ...
  \label{eqn:Baryv}
\end{align}
\todo[inline]{Need to cite work that Barnett et al. reference} 
To approximate $v'(x)$, the trapezoid rule is applied to the two
integrals in the identity
\begin{align}
  v'(x) = \frac{\int\limits_{\bd\Omega} 
    \frac{v^{-}(y) - v(x)}{(y-x)^2}\, dy}
  {\int\limits_{\bd\Omega} \frac{1}{y-x}\, dy},
\end{align}
resulting in the quadrature rule
\begin{align}
  v'(x) \approx \frac{\sum\limits_{j=1}^{N}
    \frac{v^{-}(y_j) - v(x)}{(y_j-x)^2} w_j}
  {\sum\limits_{j=1}^{N} \frac{1}{y_j-x} w_j}.
  \label{eqn:Baryvprime}
\end{align}

To compute the shear stress and vorticity, we require a Barycentric
quadrature rule for $v''(x)$.  Mimicking the construction of the
quadrature rule for $v(x)$ and $v'(x)$, we start with the second
derivative of the Cauchy integral theorem
\begin{align}
...
\begin{align}
  ...
  \right.
\end{align}
where $x \in \Omega$ and $n \in \mathbb{N}$, we have
\begin{align}
  0 = \frac{1}{2\pi i} \int_{\bd\Omega} 
...
\begin{align}
  0 \approx  \sum_{j=1}^{N} 
      ...
      {(y_j-x)^3} w_j,
\end{align}
where the accuracy is independent of the proximity of $x$ to
$\bd\Omega$. Solving for $v''(x)$, the Barycentric quadrature rule for
...
\begin{align}
  v''(x) &\approx \frac{2\sum\limits_{j=1}^N 
    ...
    {\sum\limits_{j=1}^N \frac{1}{y_j-x}w_j}.
\end{align}
This formula requires accurate approximations of $v(x)$ and $v'(x)$, but
these are available in equations~\eqref{eqn:Baryv}
and~\eqref{eqn:Baryvprime}.

To apply the quadrature to multiply-connected domains, we must all
consider exterior points $x \in \Omega^c$.  Mimicking the construction
in~\cite{bar-wu-vee2015}, we use the quadrature rule
\begin{align}
...
\end{align}
Again, values of $v(x)$ and $v'(x)$ are required, and these are
available in~\cite{bar-wu-vee2015}.
...
simulate dynamics very close to the eroding bodies (Figure~\ref{fig:}
\todo[inline]{reference a figure or two from the results section)}.
...
To calculate tortuosity $T$, we use the simple quadrature rule
\begin{align}
  T = \frac{1}{d}\frac{\displaystyle\sum_{i=1}^{N} 
    u_x(y_i) \lambda(y_i) \Delta y_i}
  {\displaystyle\sum_{i=1}^{N} u_x(y_i) \Delta y_i}, 
\end{align}
...
\todo[inline]{Talk about time integration for erosion and for tracers}
...
pressure drop from $x=-2$ to $x=2$ remains constant.  
{\color{red}
...
%\caption{}
\end{subfigure}
\caption{\label{fig:Eroding20flowrate} The incoming flow rate of $U$ in log scale versus time. 
...
\begin{figure}[H]
\begin{center}
  \includegraphics[height=0.23\textwidth]{./figs/20b_dense1}
  \includegraphics[height=0.23\textwidth]{./figs/20b_dense101}
  \includegraphics[height=0.23\textwidth]{./figs/20b_dense201}
  \includegraphics[height=0.23\textwidth]{./figs/20b_dense301}
...
bodies are illustrated in the last frame of Figure~\ref{fig:Eroding20tracer}. 
...
\caption{\label{fig:Eroding20tracer}The trajectories of 200 tracers in
  the fluid with 20 bodies. The five first snapshots are evenly spaced in time.
  The tracers are evenly spread in the $y$-direction and released at
  $x=-1$. The sixth frame is a zoom window of the fifth frames. 
Since we use a Barycentric quadrature rule
...
the fifth frame of Fig.~\ref{fig:Eroding20tracer}, some tracers
climb up from right to left around the body.  There is a zoom windows of
the climbing tracers trajectories in the last frame of Fig.~\ref{fig:Eroding20tracer} and
they are taken from the fifth frame of
Fig.~\ref{fig:Eroding20tracer} with more tracers.  In the last frame of 
...
From Fig.~\ref{fig:Eroding20tort}(c),
 we are able to tell the neighboring trajectories will be very 
different if they are separated by bodies. In Fig.~\ref{fig:Eroding20tort}(c)
...
(b) The local tortuosity $\tau(y)$ on the cross section x = -1. 
(c) The historical trajectories of 
200 tracers released at x = -1 in the fluid with multiple bodies.}
%(c) The difference of $\tau(y)$ at two neighboring tracers on the cross section x = -1.
%(d) The ratio of the minimum to maximum speeds along the trajectories of tracers.}
\end{figure}
