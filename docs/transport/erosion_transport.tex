...
transitions from ballistic dynamics at early times to asymptotic
super-diffusive dynamics.  In contrast to the lower porosity initial
condition (Section~\ref{sec:Eroding20}), at early times the erosion
results in a decrease in particle spreading. In particular, once erosion
has eroded the first 5\% of the bodies, the particle spreading
transitions from $\sigma_\lambda \sim t^{0.92}$ to $\sigma_\lambda \sim
t^{0.69}$.  To explain this behavior, note that the velocity is fairly
uniform, albeit small, at the initial configuration (see
Figure~\ref{fig:ErodingLow20vort}).  Then as the geometry erodes, the
ratio of the fast and slow regions increases.  Since super-diffusive
dispersion results from a tracers spending times in a both slow and fast
regions, this results in an increase in the rate of super-dispersion.
\todo[inline]{Still need to work on this and explain how it contrasts
from the other 20 body example}
\todo[inline]{Need a reference for last sentence}
...
\todo[inline]{do better job of explaining why diesperions does what it
does.  For this problem, there are lots more of small bodies rather than
a few big ones.  Therefore, after reinsertion, many of the bodies are
placed in a ballistic regime which means that anoamlous dispersion is no
longer observed.}
Next, we use the 1000 tracer trajectories to compute the second
(Figure~\ref{fig:Eroding100Transport}(b)) moments of the streamlines at
...
Similar to the other previous examples, we initially observe ballistic
motion, and then super-diffusive dispersion.  However, similar to the
example in Figure~\ref{fig:ErodingLow20Transport}(b), the anomalous
...
porosity, but also the location and shape of the bodies.
...
\caption{}
\end{subfigure}
\caption{\label{fig:Eroding100gap_hist} The distribution of the gaps in the
case of erosion of 100 bodies at six porosities and the fitting curves of Weibull distribution 
described by the shape parameter $k > 0$ and the scale parameter $\lambda > 0$. 
(a) 100 bodies with 258 gaps as porosity $\phi = 50.09 \%$ 
and $(k, \lambda)=(1 .4650, 0.0605)$ for Weibull distribution. 
(b) 100 bodies with 258 gaps as porosity $\phi = 55.02 \%$ 
and $(k, \lambda)=( 1.9485, 0.0737)$ for Weibull distribution. 
(c) 97 bodies with 249 gaps as porosity $\phi = 65.03 \%$ 
and $(k, \lambda)=( 2.5682, 0.0965)$ for Weibull distribution. 
(d) 92 bodies with 234 gaps as porosity $\phi = 75.14 \%$ 
and $(k, \lambda)=( 2.7771, 0.1255)$ for Weibull distribution. 
(e) 75 bodies with 189 gaps as porosity $\phi = 85.07 \%$ 
and $(k, \lambda)=( 2.6235, 0.1713)$ for Weibull distribution. 
(f) 34 bodies with 74 gaps as porosity $\phi = 95.12 \%$ 
and $(k, \lambda)=( 1.9840, 0.3194)$ for Weibull distribution. }
\end{figure}
...
In Figure~\ref{fig:Eroding100gap_mean_var},
we plot the mean and variance of the throat sizes as a function of the
porosity.  While the mean steadily grows with the porosity, the variance
remains relatively flat for a wide range of porosities. 
...
\end{subfigure}
\caption{\label{fig:Eroding100gap_mean_var}  The mean of gaps in (a) and
the variance with of gaps in (b) with respect to porosity $\phi$.  Red
stars are six selected porosities to have the details of  the
distribution of gaps and fitting curves of the distributions in
Figure~\ref{fig:Eroding100gap_hist}.}
\end{figure}
