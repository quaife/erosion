...
\begin{align}
  ...
  d(\xx,\gamma_k) = \inf_{\yy \in \gamma_k} \|\xx - \yy\| > 
    5 \frac{L_k}{N_\iin}.
\end{align}
To avoid checking if a target point $\xx$
...
\begin{align}
  ...
  \|\cc_i - \cc_j\| < \frac{L_i}{2\pi} + \frac{L_j}{2\pi} + 
    \alpha \left(\frac{L_i}{N_\iin} + \frac{L_j}{N_\iin} \right),
\end{align}
where $\alpha /geq 1$ is a free parameter that needs to be chosen.
In this manner, we are checking the proximity of the two circles with
centers $\cc_i$ and $\cc_j$, and with radii $L_i$ and $L_j$, where
$\alpha$.  This criteria allows us to quickly determine bodies that
contain discretization points where the Barycentric quadrature rule
should be applied.  Assuming that the two bodies $\gamma_i$ and
$\gamma_j$ satisfy condition~\eqref{eqn:BodyCutoff}, for each point $\xx
\in \gamma_j$, we check if
\begin{align}
  \label{eqn:PointsCutoff}
  \|\xx - \cc_i\| < \frac{L_i}{2\pi} + \alpha_\iin 
    \left(\frac{L_i}{N_\iin} \right).
\end{align}
By applying this method, we do not have to do an all-to-all comparison
of the discretization points on two eroding bodies.

To determine an appropriate value for $\alpha_\iin$, we first compute an
accurate solution by using the Barycentric quadrature rule for all
discretization points.  Then, for multiple values of $\alpha_\iin$, we
compute the velocity field with the trapezoid rule for all points that
do not satisfy~\eqref{eqn:PointsCutoff}.  By comparing these two
velocities, we find that a value of $\alpha_\iin = ???$ gives sufficient
accuracy while keeping the number of points that require the expensive
Barycentric quadrature rule to a minimum.

Given an eroding body $\gamma_i$, to determine if a target point $\xx$
that is in the fluid bulk rather than on another eroding body requires
the Barycentric quadrature rule, we only check
condition~\eqref{eqn:PointCutoff}.  Finally, to determine if points on
$\gamma_i$ are too close to the outer wall, we make use of the fact that
the outer wall is bounded in $[-3,3] \times [-1,1]$, and that the bodies
are all contained in $[-1,1] \times [-1,1]$.  Therefore, we first check
if
\begin{align}
  \|\cc_i \pm 1\| < L_i + \alpha_\out \frac{L_\out}{N_\out}.
\end{align}
If body $\gamma_i$ satisfies this condition, for each point $\xx \in
\gamma_i$, we apply the Barycentric rule to points that satisfy
\begin{align}
  \|\xx \pm 1\| < \alpha_\out \frac{L_\out}{N_\out}.
\end{align}
Using the same strategy as we used for body-body interactions, we find
that an appropriate safety parameter is $\alpha_\out = ???$.
...
\caption{A single body eroding in Stokes flow. Color indicates the shear stress on the body. In this simulation, we use three minimum body-to-wall initial distances $d=h$ (left), $h/2$ (middle), and $h/10$ (right) with $N_\iin = 256$ discretization points on the body, a time-step of $\Dt=10^{-4}$, and $\eps = 15/256$ and $\sigma=10/256$.}
\label{} 
\end{center}
\end{figure}
{\color{red} We first consider a single circular body in the shear flow. 
This example shows that we can resolve eroding bodies near the solid wall which is similar to \cite{mit-spa2017}.
The initial velocity of flow from the left side of $\Omega$ is 
$$\UU(\xx)=\begin{bmatrix} y+1 \\ 0 \end{bmatrix}.$$
We place a single eroding body close to the no-slip wall ($y=-1$).
This example has no pressure drop. 
We investigate the shape of eroding body when it is close to a solid wall at the three  
distances $d=h, h/2$, and $ h/10$ where $h=L/N_\iin$ is the arclength spacing correponding to the initial radius $r$ of 0.4 and $N_\iin=256$ discretization points . 
...
\caption{\label{fig:Eroding20vort} Erosion of 20 nearly touching bodies with the fixed pressure drop condition. The color is the vorticity of the fluid. The 4 snapshots are evenly spaced in time. We use $N_\iin = 256$ discretization points on the body, a time-step of $\Dt=10^{-4}$, and smoothing parameters of $\eps= 15/256$ and $\sigma=10/256 $. In the forth frame, bodies 4, 12, and 17 have vanished completely, and body 19 has almost completely eroded.}
\end{center}
\end{figure}
...
In Fig.~\ref{fig:Eroding20tort} we use tortuosity to analysis the effect of the geometrical structure on fluid velocity field. In Fig.~\ref{fig:Eroding20tort}(a) the graph represents the ratio of the x-component velocity u(-1, y) to its maximum velocity $u_{max}=6.2281 \times 10^{-4}$ at equal spreading tracers on the cross section (x=-1 and y in [-1,1]). The velocity is continuous along the cross section and acting as several Poiseuille flows. This velocity graph is different from the one in~\cite{matyka2008tortuosity} since we choose our cross section to be the beginning of the flow before the fluid touches any body. In Fig.~\ref{fig:Eroding20tort}(b) we record the local tortuousity $\tau$ of the tracers at the cross section. This graph is not continuous and the jumps in $\tau$ mean that the initial neighbor tracers have been seperated by the bodies. 
...
\end{subfigure}
\begin{subfigure}[b]{0.5\textwidth}
\includegraphics*[width =\linewidth]{./figs/tortU_local}
\caption{}
\end{subfigure}%
\begin{subfigure}[b]{0.5\textwidth}
