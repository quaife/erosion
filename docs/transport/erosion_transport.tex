...
This paper characterizes transport in eroded geometries by computing and
analyzing a variety of coarse-grained variables. 
...
eroded geometry often contains channels of high porosity that connect
the inlet to the outlet, and this results in velocities that vary over
several orders of magnitude~\cite{all-hea-lab-rei2002}.  Moreover,
channelized geometries are heterogeneous and anisotropic, and this
affects the transfer of quantities such as
...
eroded geometries, we must complete three main tasks.  The first two are
forming high-fidelity numerical solutions of the fluid equations, and
eroding the grains according to an erosion model. These two tasks are
performed using our previous work~\cite{qua-moo2018}, but we improve our
numerical methods so that simulations of more realistic dense
suspensions of eroding grains are possible
(Figures~\ref{fig:Eroding50vel} and~\ref{fig:Eroding50vort}).  Finally,
in this paper, we simulate and characterize transport through eroded
porous geometries.
...
the two-dimensional incompressible Stokes equations. A variety of
numerical methods are available to solve the incompressible Stokes
equations including lattice-gas or lattice
...
method that is able to resolve geometries with arbitrarily shaped and
nearly-touching grains.  Therefore, we solve the incompressible Stokes
equations with a boundary integral equation (BIE).  To couple erosion
...
\begin{align}
  ...
  \label{eqn:tracers}
\end{align}
where $\uu$ is the fluid velocity, and the trajectories are simply the
streamlines. Since there is no stiff diffusive term
in~\eqref{eqn:tracers}, we use a high-order explicit Runge-Kutta time
stepping method. Other methods to compute streamlines include a
...
\caption{\label{fig:Eroding50vort} The same six snapshots from
...
apply a stable second-order Runge-Kutta method is applied to the {\thL}
coordinates~\cite{hou-low-she1994} of the eroding grains.  In these
coordinates, the stiff term is linear and treated implicitly, so we are
able to take large time step sizes. In our previous work, after
discretizing the BIE, the trapezoid rule is used to approximate the
integrals. The accuracy of the trapezoid rule is sufficiently accurate
if all the bodies are sufficiently separated~\cite{tre-wei2014}, but
nearly-singular integrals must be computed when two grains are
nearly-touching or when evaluating the velocity or vorticity near an
eroding grain.  One of the earliest works on near-singular integration
...
guaranteed to be uniformly bounded.  We extend the original quadrature
method so that it can be used to compute the gradient of the velocity
that is required to compute the shear stress and the fluid vorticity. In
addition, the quadrature method is used to accurately compute streamline
trajectories that pass through the very slow regions close to the
eroding grains.
...
spreading, the fluid dispersion is computed as the variance of the
streamline lengths. In porous media, this spreading can often be
...
quadrature method, we accurately compute erosion rates and stably
simulate erosion.  Moreover, we use the quadrature method to visualize
the vorticity in the fluid bulk.  The vorticity is convenient to
visualize since, in the zero Reynolds number limit, it is equivalent to
the shear stress on the eroding bodies, and therefore its magnitude is
the erosion rate.

Our second contribution is computing and analyzing trajectories in the
eroded geometries.  These trajectories are used to characterize the
transport by computing local tortuosities of the trajectories, the
tortuosity of the eroded geometry, and the anomalous dispersion rates in
the geometry.  By using the same quadrature rule and a fourth-order
Runge-Kutta method, we are able to accurately compute trajectories that
spend large amounts of time in the slow velocity regions near the
eroding bodies, and computing these trajectories reliably is essential
to characterize the flow~\cite{dea-leb-den-tar-bol-dav2013}.  We compare
...
immobile eroding grains. However, this new work does not have a limitation on
the proximity of the bodies. Compared to our previous work, we are able to
...
transport in this work, we only consider passive particles that are only
...
this point, a new Delaunay triangulation is formed.  {\color{red}In
Figure~\ref{fig:Eroding100gap_mean_var}, we record the gap distances
between bodies neighbors and calculate the mean and variance with
respect to porosities.  For the mean of gap distances, it is linearly
increacing until $\phi = 85\%$ which means the eroding .  For the
variance of gap distances, it is almost a constant function until $\phi=
85\%$  }
\begin{figure}[H]
\begin{subfigure}[b]{0.5\textwidth}
...
histograms of the pore throat sizes at six porosities of the erosion
