...
flux. This arrangement results in velocities that vary over several
...
developed to simulate erosion in the regime of Stokes flow
...
resolve dense suspensions of eroding grains.  The accuracy of the
trapezoid rule, which we used in our previous work, is adequate for
...
To characterize the transport, particle trajectories must be computed.
Depending on the application, microscale transport can be modelled as
...
geometries. Finally, we analyze the effect of erosion on the pore size
distribution.
...
between solving the fluid equations nd advancing the grains according to
the erosion rate for a small time step. The strength of $\UU$ is
adjusted at each time step to achieve a constant pressure drop across
the channel.
...
addressed with the integral operator $\NN_0$ which is the integral
operator with kernel $N_0(\xx,\yy) = \nn(\xx) \otimes \nn(\yy)$,
...
$\gamma$, the shear stress is computed as defined in
...
and $\eta$ is the complex counterpart of $\eeta$.
...
potential representation of the gradient of the velocity.  The
