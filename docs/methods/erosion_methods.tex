...
\dot{\xx}(t) = \Vn \nn + \Vs \ss \, .
\end{align}
Since $\Vs$ is tangential, it does not modify the erosion law \eqref{eqn:Vn}, only the collocation grid.
...
\thalpha \Vs \right).
\end{align}
System \eqref{eqn:Levo1}--\eqref{eqn:thetaevo1} comprise the main evolution equations to be solved numerically in Section~\ref{sec:timeStepping}. 

In addition, it is necessary to track a reference point for each body, so that the variables $\theta$ and $L$ can be converted back to cartesian coordinates. We track the coordinates of the surface mean $\mean{\xx}$, which moves according to
\begin{align}
\label{eq:xsurf}
\frac{d}{dt} \mean{\xx} = \mean{ V_s \ss + V_n \nn }
\end{align}
We note that tracking the center of mass is also a valid choice, but the surface mean gives a somewhat simpler formula.
...
\todo[inline]{Mention key term exponential integrator for stiff term.}
