...
As a final validation, we now aim to verify the second-order convergence rate (in time) of our method. To this end, we run the same single-body simulation from Figs.~\ref{01bodseq} and \ref{shrink_intface} to a stopping time of $t_s = 1E-2$, which is 56\% of the time it takes for the body to vanish completely. By this time, the body has eroded substantially, but is not so small that accuracy is lost. We assess error by measuring the difference between simulations of successive resolution (i.e.~we verify Cauchy convergence). In particular, we measure the $L^2$-difference of the body shapes obtained at the stopping time, $t_s$, while successively doubling the temporal resolution. Table \ref{convtab} shows the $L^2$-errors between successive simulations, along with the order of accuracy as obtained by Richardson-extrapolation-type calculations. The table confirms that our method to converges with second-order accuracy.
