...
Briefly, to confirm that the rescaling does not alter accuracy, we show HERE a convergence test for 20 bodies with the condition of a fixed pressure drop.

% Table
%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%
\begin{table}%[htbp]
\begin{center}
\caption{Convergence test for multiple-body erosion with fixed pressure drop.
}
\vspace{0.3 pc}
\label{convtab}
\begin{tabular}{l l l l}
