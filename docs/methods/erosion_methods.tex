...
\todo[inline]{I am not sure if $V$ has been defined previously. If not, need to define it.}

Interestingly, a brief look at Fig.~\ref{shrink_intface}(a) reveals that the spacing between successive interfaces becomes larger as the body shrinks, suggesting that the overall shear stress is growing. Of course, such a relationship is consistent with the well-known 3D Stokes drag law, $\tau \sim \mu V / L$, since stress is inversely proportional to body scale $L$. This law must be modified in 2D, though. The modification is made non-trivial by subtleties associated with the Stokes paradox, but can be correctly deduced by noting that, as the body vanishes, its drag must be consistent with slender-body theory. For a slender body in flow perpendicular to its long-axis $W \gg L$, the drag is proportional to $\mu V W/ \log(W/L)$. This relationship implies that, for a 2D cross-section perpendicular to the long axis, the stress can be estimated by
\begin{equation}
\label{eq:stresslaw}
