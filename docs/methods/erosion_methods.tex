...
\begin{equation}
\mu_k^{(n+1/2)} = \exp \left( \eps \pi^2 k^2 \Dt \left( \elfun^{(n)} + \elfun^{(n+1/2)} \right) \right)
\end{equation}
...
\exp \left( - \eps \pi^2 k^2 \Dt \left( \elfun^{(n)} + \elfun^{(n+1/2)} \right) \right)
\end{equation}
...
& \frac{ \mu_k^{(n+1)} \thhat_k^{(n+1)} - \thhat_k^{(n)}}{\Dt} = \hat{N}_k^{(n+1/2)}
\end{align}
We again compute the integrating factor, $\mu_k^{(n+1)}$ via the trapezoid rule, giving
\begin{equation}
\thhat_k^{(n+1)} = \left( \thhat_k^{(n)} + \Dt \, \hat{N}_k^{(n+1/2)} \right)
\exp \left( - 2 \eps \pi^2 k^2 \Dt \left( \elfun^{(n)} + \elfun^{(n+1)} \right) \right)
\end{equation}
