...
many bodies (order 50) in a two-dimensional Stokes flow. The broad aim
...
boundary integral formulation of the Stokes equations. We solve the
associated linear system with a fast-multipole-accelerated GMRES iteration. We use a second-kind integral equation so
that GMRES requires a bounded iteration count (independent of the number
of unknowns). To evolve the individual grains under shear-driven
...
fluid solver requiring $\mathcal{O}(N)$ operations per GMRES iteration, and a one-time
