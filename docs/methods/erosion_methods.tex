...
method.  
\todo[inline]{Maybe point out some drawbacks of using one of these BIE methods that allows corners. For example, are these methods more conceptually complex and/or more expensive? Maybe they are not quite at a mature stage for application? Another point: in the simulations we cannot predict where corners will form and so would have to identify them on the fly. Would this cause a problem if we tried to use a corner-resolving BIE?}
\todo[inline]{Could say our approach is simpler because  by smoothing corners, we can use the bare-bones BIE methods that are more well established compared to these newer, corner-resolving methods. Just some thoughts.}
...
\todo[inline]{Do you think we should flesh out this time-scale stuff more precisely? i.e.~By setting $\mu = 1$ and $C = 1$ (the erosion constant), what exactly is the timescale that defines our dimensionless time.}
\begin{align}
...
\begin{align}
\label{area_predict}
\frac{A}{A_0} \left( 1 - c_1 \log{\frac{A}{A_0}} \right) \sim \frac{t_f - t}{t_f}
%A (c_1 - \log A) \sim c_2 (t_f - t) \qquad \text{as } t \to t_f \, .
\end{align}
where $c_1$ is a constant.
This is an {\em implicit} relationship for how the area scales down in time during erosion. In Fig.~\ref{area_drag}a, we show the body area as a function of time measured in the simulation against the above scaling law with $c_1 = 0.24$. The comparison shows close agreement between the two (they are indistinguishable).

In Fig.~\ref{area_drag}(b), we show the horizontal drag $F_D$ measured on the body as it erodes. This drag is normalized by a characteristic value $F_c = A_0/t_f$.
Note that, in the context of 3D slender-body theory, $F_D$ is drag per unit span. The figure shows the drag to decrease  gradually at first, then abruptly tend to zero in the final moments of the body vanishing. This behavior is consistent with fact that drag vanishes {\em logarithmically} with $L$  in 2D Stokes flow. We also show in the figure the separate contributions of the viscous and the pressure drag. Initially, these two contributions are comparable, with a slightly greater level of pressure drag. However, as the body erodes and assumes a more slender form, the pressure drag vanishes much more rapidly than viscous drag.

As a final check, are scaling estimates for area change can be rearranged to estimate the viscous drag component, $F_{\mu} \approx \mean{\atau} L$, which gives
\begin{align}
\label{viscdrag}
\frac{F_{\mu}}{F_c} \sim \frac{1}{1 - c_1 (1 + \log{A/A_0})}
...
We now discuss some physical aspects of multi-body erosion revealed by our simulations. In Fig.~\ref{fig:06bodies} we show a simulation of 6 eroding bodies, each initially circles of various sizes and positions. Over time, erosion not only diminishes the size of the bodies, but also alters their shapes considerable. As in the single-body simulations, corner-like features develop, but not in locations that are easily predictable (such as the foremost and aft-most points on the body). Rather, when and where these sharp features develop depends on inter-body feedback as mediated by the Stokes flow.
...
The initial configuration in  Fig.~\ref{fig:06bodies} was selected to have several near contacts between bodies, for example between bodies 1 \& 2, 3 \& 5, and 5 \& 6. Our method handles these near contacts without difficulty. Interestingly, erosion appears to preferentially remove material from these regions of near contact, as the narrow spacing between bodies expands rather rapidly. For example, the spacing between bodies 1 \& 2 was initially very small but is already comparable to the spacing between bodies 3 \& 4 by the second frame. Even more interesting, the bodies tend to develop flat faces wherever near contact occurs, which leads to the appearance of straight channels in between bodies. Such channels are apparent between bodies 1 \& 2, 3 \& 5, and 5 \& 6, and they {persist} for the duration of the simulation.

The vorticity field (color) is useful to infer both the regions of highest shear and the regions of most intense flow. Figure~\ref{fig:06bodies} shows that the flow is concentrated primarily in a single channel running horizontally, below bodies 1 \& 2 and above bodies 3 \& 6. This main channel widens the fastest as the high-flow rate leads to greater shearing on the boundaries. As discussed above, the pressure drop across the cell is fixed in these simulations, and so as the bodies diminish the flow rate increases, which can be seen by the increase in the vorticity magnitude. By the final frame shown, body 4 has vanished completely and our method handles this event seamlessly.
