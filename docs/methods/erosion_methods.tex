...
for the fluid equations.  We will start by writing the velocity field in
terms of the standard double-layer potential~\cite{laz1963,poz1992}.
Since the double-layer potential representation is only able to
represent force-free and torque-free velocity fields, Power and
Miranda~\cite{pow-mir1987} completed the BIE formulation by introducing
a Stokeslet and Rotlet.  This methodology was extended by Power for
multiply-connected domains~\cite{pow1993}.  In two-dimensions, a stream
function formulation can be used in which case the biharmonic equation
must be solved, and this can be done using BIEs by using analytic
function theory~\cite{gre-kro-may1996}.
