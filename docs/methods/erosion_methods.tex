...
\end{figure}
 %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%
% Figure generated using 20circ128.in with epsfac = 8, sigfac = 8, dt = 2e-4, fixpdrop = 1.
% Figure was made after the code changes regarding scaling the smoothing on the average stress on each body.
...
\todo[inline]{Check theta equation}
\begin{align}
...
\begin{figure}%[htbp]
\begin{center}
\includegraphics[width = 0.99 \textwidth]{./figs/01shrink_plots.pdf}
\caption{One body}
\label{}
\end{center}
\end{figure}
...
\begin{figure}%[htbp]
\begin{center}
\includegraphics[width = 0.99 \textwidth]{./figs/01fix_plots.pdf}
\caption{One body fixed area}
\label{}
\end{center}
\end{figure}
 %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%
% Figure from datafile 01circ1024cc
% Uses 01circ1024.in with epsfac = 10, sigfac = 10, dt = 1e-6, fixarea = 1, fixpdrop = 0
