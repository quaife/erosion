...
stress~\cite{ris-moo-chi-she-zha2012}.  However, tracking the interface of
fluid-structure interaction problems with a long time horizon is
particularly difficult---the mesh is often distorted and tangled by the dynamics
...
accuracy.  All spatial derivatives are computed in Fourier space,
...
use a BIE for the traction jump to simulate the erosion of a mobile body
in three dimensions, but they only consider single body.  
...
porous region as a function of time can be computed.
...
Another numerical challenge with simulating erosion is that
corners develop at points where the shear stress vanishes.  There is recent work for solving
...
Figure~\ref{shrink_intface}(a) shows that the spacing between successive interfaces becomes larger with time, suggesting that the overall shear stress grows as the body vanishes. This observation is consistent with the well-known 3D Stokes drag law, in which stress, $\tau \sim \mu \umax / a$, scales inversely proportional to body-scale $a$ (e.g.~for a sphere, $a$ is taken to be the radius). This law must be modified in two dimensions, though. The modification is made non-trivial by subtleties of the 2D Stokes paradox, but can be correctly deduced by noting that, as the body vanishes, its drag must be consistent with slender-body theory \cite{poz1997, bouzarth2007epicyclic, camassa2008trajectory, MooreJFM2012, zhao2015transportation}. 
...
Here, time is normalized by the same vanishing time $t_f = 1.79 \times 10^{-2}$ as in Fig.~\ref{shrink_intface}. Curiously, about 3 multiples of $t_f$ are required to reach an approximate steady-state, which contrasts with previous studies of high-Reynolds-number erosion, in which a satisfactory steady-state was usually reached before the body vanished~\cite{moo-ris-chi-zha-she2013}.
