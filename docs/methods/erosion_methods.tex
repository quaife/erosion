...
Erosion by flowing fluid plays an essential role in diverse processes.
Many of the flows are geophysical, such as the formation of sand dunes
or erosion due to groundwater flow.  In both these examples, there is a
feedback process between the flow and the erodible body.  In particular,
the shape of the eroding body affects the flow, while the flow changes
the shape of the eroding body.  This fluid-mechanical feedback
system has be simulated in idealized cases such as a single eroding body
in a uniform flow~\cite{moo-ris-chi-zha-she2013}.  To further develop
insight into the effect of two-dimensional erosion, we consider erosion
of a multiply-connected domain (Figure~\ref{fig:50bodies}), such as a
porous media that is typically used to model groundwater flow.

Because of the scales involved with groundwater flow, we will restrict
our attention to Stokesian dynamics.  That is, the fluid equations will
be governed by the incompressible Stokes equations.  Because of the
linearity of the underlying equations, a boundary integral equation is
well-suited as it can resolve complex geometries, the interactions
between bodies, all the while achieving high-order accuracy and
computational efficiency.

\paragraph{Contributions} The present study will develop numerical
methods to simulate viscous erosion of a multiply-connected
two-dimensional geometry.  By assuming the flow is viscous, we are
...
multiply-connected domains, the erosion caused by the non-local complex
interactions between different bodies is modeled.

Because of the low Reyonlds number assumption, the flow will be assumed
...
Mitchell and Spagnolie~\cite{Mitchell2016} simulate the erosion of a
...
\caption{\label{fig:50bodies} 50 bodies with vorticity}
\end{center}
\end{figure}
 %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%
...
%In this section, we also describe our time stepping method and two regularization terms that are necessary to control the curvature of the bodies and to maintain spectral accuracy in space.

%Once the shear stress is computed, it is used to update the shape of the bodies according to the normal flow~\eqref{eqn:bodyEvolution}.  To eliminate tangling and stretching of the mesh, we use a {\thL}formulation~\cite{hou-low-she1994} which is described in Section~\ref{sec:thetaL}.
...
%\frac{4\pi^2}{L^{2} \log \left(2\pi/L \right)}
\end{align} 
...
\end{align}
Recall that $\alpha = 2 \pi s / L$ is a normalized arc length.
