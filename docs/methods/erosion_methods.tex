...
This choice is based on experimental work of Ristroph et
...
\end{align}
\todo[inline]{multiply defined references here}
...
equations~\eqref{eqn:Levo2}--\eqref{eqn:thetaevo2} as
\begin{align}
\label{eq:L12}
...
\label{sec:NSI}
To accurately compute the velocity~\eqref{eqn:velocityDLP},
vorticity~\eqref{eqn:vorticityDLP}, and pressure~\eqref{eqn:pressureDLP}
at points $\xx \in \Omega$, we adopt a near-singular integration scheme
entirely based on upsampling.  Since we are only interested in computing
the velocity and vorticity for visualizing the flow, and the pressure is
only required far away from any boundary, only a finite amount of
upsampling is required to achieve an acceptable error.
