...
\begin{align}
  \uu(\xx) = (1-y^2,0)
\end{align}
on $\Gamma$.  To minimize the boundary effects of the inflow and
outflow, we only place bodies in the center third, $[-1,1] \times
[-1,1]$, of the fluid domain.
\begin{figure}[htpb]
  \centering
...
illustrated in Figure~\ref{fig:workflow}.
...
equation~\eqref{eqn:completed_DLP} which we remove by adding the 
rank-one modification $\NN_0[\eeta](\xx) = \int_{\Gamma_0} (\nn(\xx) \times
...
stress which is defined on the boundary of each body.  The shear stress
is given by
\begin{align}
  \tau = \left(\nabla \uu + \nabla \uu^T \right)\nn \cdot \ss,
...
FMM,~\eqref{eqn:completed_DLP} can be solved with $\mathcal{O}(N)$
