...
With the validation complete, we now discuss some of the emergent physics of multi-body erosion revealed by our simulations. We begin with a relatively simple case of 6 eroding bodies, as this will allow us to identify key principles at work in more complex cases (e.g.~the 50-body simulation in Fig.~\ref{fig:50bodies}). Figure~\ref{fig:06bodies} shows the simulated erosion of 6 bodies, each initial circles of various sizes and positions. Over time, erosion not only diminishes the size of the bodies, but also alters their shapes considerable. As in the single-body simulations, corner-like features develop, but not in locations that are easily predictable. Rather, when and where these corners form depends on how neighboring bodies interact as mediated by the Stokes flow.
...
The initial configuration in  Fig.~\ref{fig:06bodies} was selected to have several points at which the bodies nearly contact one another, for example bodies 1 \& 2, 3 \& 5, and 5 ur \& 6. Intriguingly, erosion appears to remove material preferentially from these near-contact regions, as the narrow spacing between neighboring bodies expands rather rapidly. For example, the spacing between bodies 1 \& 2 is initially very small, but by the second frame, is comparable to the spacing between bodies 3 \& 4. Furthermore, the bodies tend to develop flat faces at the locations of near contact, leading to the appearance of straight channels between bodies. Straight channels can be seen, for example, between bodies 1 \& 2, 3 \& 5, and 5 \& 6. Once formed, these channels persist until the bodies have nearly vanished. Notice that by the final frame, body 4 has vanished completely, and the simulation handles this event without difficulty.

The vorticity field helps not only to visualize regions of high shear, but also to identify the certain preferred channels in which the flow is most intense. As seen in Fig.~\ref{fig:06bodies}, the flow is concentrated primarily in a single channel running horizontally below bodies 1 \& 2 and above bodies 3, 5, \& 6. This main channel widens the fastest, and secondary channels (e.g.~between bodies 1 \& 2 or 3 \& 5 or 5 \& 6) follow. Notice that, in these simulations, the overall vorticity magnitude increases in time as a result of fixing the pressure-drop across the cell.
