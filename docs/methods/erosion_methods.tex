...
We now aim to measure the opening angle formed in our simulations for comparison with the above prediction. However, Section \ref{sec:scaling} shows that the magnitude of the shear stress diverges as the body vanishes, which causes our simulations lose accuracy in the critical moments when the body is expected to approach its limiting shape. To circumvent this problem, we remove the effect of shrinking by forcing $\Vn$ to have zero mean,
\begin{align}
