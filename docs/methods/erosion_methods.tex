...
\begin{align*}
  \xx^k(\alpha) = \sum_{j = -N_\iin /2}^{N_\iin /2-1} 
    \hat{x}_j^k e^{ij\alpha}, \quad k=1,\ldots,M.
\end{align*}
...
\begin{align*}
  \frac{d\xx^k(\alpha)}{d\alpha} = 
    \sum_{j = -N_\out /2}^{N_\out /2-1} ij\hat{x}_j^k e^{ij\alpha},
    \quad k=1,\ldots,M.
\end{align*}
...
\end{align}
\todo[inline]{Earlier, I assumed the geometry is $2\pi$ periodic.  Is it
possible to use this convention cleanly for the presentation?  This
allows us to avoid the long superscripts $2\pi i k \alpha$}
...
\end{align}
We solve system~\eqref{eqn:Levo2}--\eqref{eqn:thetaevo2} for the
variables $L$ and $\{ \mu_k \theta_k \}$ with a 2nd-order Runge-Kutta
method. At each stage, we must recover $\theta(\alpha,t)$ from  $\{
  \mu_k \theta_k \}$ in order to compute the shear stress as needed in
  Eqs.~\eqref{eq:M} and~\eqref{eq:N} for $M$ and $N$. For this, we
  compute $\mu_k$ from Eq.~\eqref{eq:mu} using the trapezoid rule, then recover $\theta(\alpha, t)$ via a Gaussian filter....
