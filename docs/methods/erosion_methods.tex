...
\end{align}
The parameter $\eps \ll 1$ controls the strength of the curvature-penalization. This term causes regions of high curvature to recede more rapidly and thus smooths out any high-frequency oscillations that may exist. Above, $\mean{\cdot}$ indicates a boundary mean. Since a closed body has mean curvature $\mean{\kappa} = 2\pi / L$, the penalty term has mean zero and and thus preserves area. The only source of material loss is therefore the shear-stress itself. In addition, we have scaled the penalty term on the mean absolute shear, $\mean{\atau}$, so that the ratio of smoothing-to-erosion always remains fixed, even as bodies vanish.
...
\dot{\xx}(t) = \Vn \nn + \Vs \ss \, .
\end{align}
Since $\Vs$ is tangential, it does not modify the physics of erosion, only the grid spacing.
...
\end{align}
We note that tracking the center of mass is also a valid choice, but the surface mean gives a somewhat simpler formula. Equations \eqref{eqn:Levo1}--\eqref{eq:xsurf} comprise the main system of equations to be solve numerically in Section~\ref{sec:timeStepping}. 
...
With the shear stress computed, we are now ready to numerically evolve the boundaries using the {\thL} framework. The main equations to be solved are \eqref{eqn:Levo1}--\eqref{eq:xsurf}.  In Section~\ref{sec:thetaL} we introduced a curvature-penalization term into the erosion law \eqref{eqn:Vn}. This term smooths out high-frequency oscillations, but also introduces {\em stiffness} into evolution equation \eqref{eqn:thetaevo1} for $\theta(\alpha,t)$. For numerical stability, we will treat the stiff term with an exponential integrator, while handling all other terms in \eqref{eqn:Levo1}--\eqref{eq:xsurf} with an explicit Runge-Kutta method. Ultimately, we will arrive at a method that is second-order in time.

First, we must address a second potential source of numerical instability. The erosion law \eqref{eqn:Vn} depends on the {\em absolute value} of shear stress, $\atau$, which exhibits a corner anywhere $\tau$ changes sign (a stagnation point). This corner in $\atau$ introduces high frequencies into \eqref{eqn:thetaevo1} which could result in either numerical instability or an extremely stringent step-size restriction. To avoid these difficulties, we smooth $\atau$ via a narrow Gaussian filter of width $\sigma$
\begin{align}
...
\end{align}
The first term on the right-hand-side of \eqref{eqn:thetaevo2} is the stiff term that results from the curvature penalization. This term is multiplied by $\zeta$, which is spatially constant but time dependent (due to $L$ and $\mean{\atau}$). The second term, $\NLterm$, represents the rest of the evolution equation, including contributions from the shear-dependent erosion law and from the artificial tangential velocity. Since the local shear stress depends on interactions between bodies, $\NLterm$ is both a nonlinear and nonlocal.

We will evolve $\theta(\alpha,t)$ spectrally and so introduce the Fourier series
\begin{align}
\label{thetaSeries}
...
\end{align*}
These ODEs can be solved by applying the integrating factor
\begin{align}
\label{eq:mu}
\mu_k = \exp \left( \eps k^2 \int \elfun \, dt \right) \, ,
\end{align}
which leads to the coupled system of ODEs
\begin{align}
\label{eqn:Levo3}
& \tderiv{L}{t} = \Mterm, \\
\label{eqn:thetaevo3}
