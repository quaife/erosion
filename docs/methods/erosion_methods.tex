...
assuming the flow is very slow, such as in groundwater flow.  Moreover,
by allowing for complex multiply-connected domains, the erosion caused
by the non-local complex interactions between different bodies is
modeled.
...
\caption{50 bodies with vorticity}
\label{}
\end{center}
\end{figure}
 %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%
