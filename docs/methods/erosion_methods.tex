...
%The constant of proportionality of the erosion law only sets the time scale, so we have set $\Vn = |\tau|$ as described in equation~\eqref{eqn:erosionModel}.  
%\todo[inline]{The above sentence is awkard. Could it be rephrased? First, the reader probably does not know what constant of proportionality you are referring to because we do not have one in the equation. Second, there are too many 'ofs' in a row.}
...
\end{figure}
%\todo[inline]{Here is an example where you can avoid the passive voice because it is totally unnecessary. Instead of saying 'the bodies are eroded', simply say 'the bodies erode'. Also, a fluid cannot be Stokesian, since the Reynolds number depends on the length scale and velocity scale. It is the flow that is Stokes.}
...
%\todo[inline]{There is a typo in the above sentence (missing 'we'?). Also, technically there is  a dangling participle; i.e.~is it the erosion model which is in Section 2.3 or the modification? A slight rephrasing might clarify.}
...
\begin{align}
  \sum_{\xx_j \in \Gamma} (\nn(\xx_i) \otimes
      \nn(\xx_j))\eeta(\xx_j) \, ,
\end{align}
is added to~\eqref{eqn:discretized_DLP}.  Since we are discetizing integrands that are smooth and periodic, the trapezoid rule employed above achieves spectral accuracy~\cite{tre-wei2014}.  
%We have applied the trapezoid rule since all the integrands are smooth and periodic, and this guarantees that the numerical solutions converges to the true solution with spectral accuracy~\cite{tre-wei2014}.
%\todo[inline]{The above sentence strikes me as awkward and verbose. Rephrase? \\
%How about this: Since all integrands are smooth and periodic, the trapezoid rule employed above achieves spectral accuracy.}
...
%However, this calculation is only performed once per time step as opposed to the velocity double-layer potential, where we do employ the FMM, that must be evaluated at each GMRES iteration. 
%\todo[inline]{The above sentence is confusing. Try to rephrase and shorten it.}
...
\todo[inline]{Aren't the CPU scaling times we report obvious.  If we take a time step size half the size, it will of course take twice as long.  We report these numbers in a couple of tables.  We have not reported CPU time versus the spatial resolution which is where the value of the FMM would be obvious.}
