...
double-layer potential~\eqref{eqn:shearStressDLP} has a singularity that
is asymptotic to $\bigO(\rho^2)$ when $\yy \rightarrow \xx$.  The
strength of this singularity can be reduced  by using the fact that a
velocity field due to a constant density function is
constant~\cite{poz1992}.  Therefore, the first part of the deformation
tensor can be evaluated as 
\begin{align*}
  \ssigma^{\DD}(\xx) = \int_{\bd\Omega}K_\ssigma(\xx,\yy)
      (\eeta(\yy) - \eeta(\xx)) \, ds_{\yy}, \quad \xx \in \bd\Omega
\end{align*}
\todo[inline]{haven't said what $K_\ssigma$ is}
