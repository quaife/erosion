...
In Fig.~\ref{fig:06bodies}, we also show the vorticity field of the surrounding flow. Not only does this field help visualize regions of high shear, but it also allows us to identify certain preferred channels in which the flow is most intense.
As seen in the figure, the flow appears to be concentrated primarily within a single channel that runs horizontally below bodies 1 \& 2 and above bodies 3, 5, \& 6. This main channel widens the fastest, and secondary channels (e.g.~between bodies 1 \& 2 or 3 \& 5 or 5 \& 6) follow. Notice that, in these simulations, the overall vorticity magnitude increases in time due to $\umax$ increasing from keeping the pressure drop fixed.
...
To further quantify these observations, Fig.~\ref{fig:mbodyplots} shows several measurements taken from the multibody simulations. In all cases, the black curves correspond to the 6-body simulation and gray curves correspond to the 50-body simulation that was shown in Fig.~\ref{fig:50bodies}. First, Fig.~\ref{fig:mbodyplots}(a) shows the area fraction of the solid bodies versus time (i.e.~the 2D counterpart of the volume fraction).
