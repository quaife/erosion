...
opportunities and challenges. First, the medium itself is composed of
many solid constituents, so multi-body interactions, especially close
interactions, must be considered. Second, the long-range interactions
...
(BIE).  Since the boundary conditions are Dirichlet, we use a
...
Furthermore, the frame of reference is changed to \thL~variables so that the curvature term
