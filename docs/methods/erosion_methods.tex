...
Next, we aim to measure the opening angle that develops at the front and rear of the body. Since these simulations are actually smooth on a fine scale, a true corner does not exist and we must instead estimate the apparent angle that results from a small region of high curvature. To this end, we first fit the tangent angle $\theta(\alpha)$ with a 7th-degree polynomial over a region that excludes a small vicinity of the front/rear stagnation points ($\alpha = 0, 0.5$). We then extrapolate the polynomial to the stagnation points which gives an estimate of the angle there. This protocol is subject to some uncertainty, which we will examine further below.
