...
To compare with this analytical prediction, we aim to measure the opening angles formed in our numerical simulations. Section \ref{sec:scaling}, however, shows that the magnitude of the shear stress diverges as the body vanishes, which causes our simulations lose accuracy just in the final critical moments when the body is expected to approach its limiting form. To circumvent this problem, we modify the normal velocity $\Vn$ to have zero mean,
\begin{align}
