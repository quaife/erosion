...
\todo[inline]{Bryan is here in editing}
...
flow~\cite{dea-qua-bir-jua2018, bar-mar-vee-zha2016}.  For nearly
...
The odd-symmetric solution to \eqref{fODE} that corresponds to $\lambda=2$ is given by
\begin{align}
...
We now aim to measure the opening angles formed in our numerical simulations to compare against this prediction. However, Section \ref{sec:scaling} shows that the magnitude of the shear stress diverges as the body vanishes, which causes our simulations to lose accuracy in the final, critical moments when the body is expected to approach its limiting shape. To circumvent this problem, we remove the effect of shrinking by forcing $\Vn$ to have zero mean,
\begin{align}
...
% Uses 01circ1024.in with epsfac = 5, sigfac = 5, dt = 2e-4, fixarea = 1, fixpdrop = 0
	...
	We show in Fig.~\ref{fixed_intface}(a) several interfaces of this modified erosion process, with the same spatial resolution, $N_\iin = 1024$, as before. Notice the body obtains a similar profile as observed previously, but with fixed area. Importantly, more numerical accuracy is now available to quantify the terminal state. The inset in Fig.~\ref{fixed_intface}(a) is a zoom showing that, due to our numerical regularization, the near corner at the body's rear is actually smooth on a fine scale. We show in Fig.~\ref{fixed_intface}(b) the stress distributions on the same interfaces. As observed before, the stress becomes more evenly distributed as erosion reshapes the body. The slight rise in shear near the front/rear corners that was observed in Fig.~\ref{shrink_intface}(b) is no longer present, suggesting that this effect was due to loss of accuracy. Rather, the late-time stress distributions in Fig.~\ref{fixed_intface}(b) decrease monotonically to zero near the stagnation points. In this figure, time is normalized by the same value $t_f = 1.79 \times 10^{-2}$ as in Fig.~\ref{shrink_intface}.

%Curiously, about 3 multiples of $t_f$ are required to reach an approximate steady-state, which contrasts with previous studies of high-Reynolds-number erosion in which a satisfactory steady-state was usually reached before the body vanished~\cite{moo-ris-chi-zha-she2013}.
...
These fixed-area simulations enable us to precisely characterize the changing geometry as it approaches a terminal state. In particular, we measure the aspect ratio of the body (the width to height) and the opening angle of the front/rear corners. In these tests, we keep the spatial resolution fixed, $N_\iin = 1024$, and show three different cases of the smoothing parameters ($\eps = \sigma = 20/1024, 10/1024,$ and 5/1024) so that we can assess how the numerical smoothing influences shape. Figure\ref{fig:arangle}(a) shows the aspect ratio as it varies in time. The initially circular geometry has an aspect ratio of unity, and, as erosion carves a more slender form, the aspect ratio increases monotonically. The three simulations are all very close to one another, which demonstrates convergence as the smoothing parameters are reduced. In the most accurate simulation (i.e.~the least smoothing), the aspect ratio tends to a value of 2.7 in long time.

Next, we aim to measure the opening angle that develops at the front and rear of the body. Since these simulations are actually smooth on a fine scale, a true corner does not exist and we must instead estimate the apparent angle due to a small region of high curvature. To this end, we first fit the tangent angle $\theta(\alpha)$ with a 7th-degree polynomial over a region excluding the front/rear stagnation points ($\alpha = 0, 0.5$). We then extrapolate the polynomial to the stagnation points, which gives an estimate of the angle there. Although it is subject to some uncertainty (to be examined below), we have found this strategy to give robust angle measurements. In Fig.~\ref{fig:arangle}(b), we show the opening angle on the eroding body for the same three cases of smoothing parameters. The method gives an angle measurement of $180^{\circ}$ for the initially circular geometry, thus providing some confirmation. As time moves forward, erosion carves a sharper corner and the angle decreases monotonically. Here, the effect of the smoothing parameters is more visible, but we still see convergence as $\eps, \sigma \to 0$. In the most accurate simulation (black), the opening angle converges to a value of $102^{\circ}$, which agrees precisely with the analytical prediction from Section \ref{LimitingShape}.

%As seen in Fig.~\ref{fig:arangle}a, the opening angle is initially $180^\circ$, corresponding to the initially circular geometry. Erosion causes the angle to form after a short time, and the angle becomes increasingly sharp as it converges to the value $102^\circ$ in the most resolved simulation. This coincides exactly with the valued predicted by our local solutions.

To examine in more detail the influence of the smoothing parameters, we show in Table

We now examine in detail the influence of the smoothing parameters and the uncertainty in the angle measurements...


\todo[inline]{Here in revising}


To examine the influence of the smoothing parameters, $\eps$ and $\sigma$, we show in Table~\ref{table:arangle} the measurements of the aspect ratio and the opening angle for different simulations. We fix the total number of discretization points at $N_\iin = 1024$ and sequentially reduce the smoothing with $\eps$ and $\sigma$ always equal. We choose the values $20/1024$, $10/1024$, and $5/1024$.  Loosely, this corresponds to smoothing over the nearest 20, 10, and 5 grid points respectively. As seen in the table, the aspect ratio converges to a values of $2.65$ and the opening angle converges to $102^{\circ}$. We also estimate the uncertainty in the angle measurements by varying the region of the polynomial fit and the degree of the fit. As shown in the table, as the smoothing parameters are reduced, the angle measurements become more precise.
...
% npts = 1024
 ...
 %A comment on how these results compare to erosion in 3D Stokes flow is in order.
