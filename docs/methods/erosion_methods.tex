...
As a final validation, we now aim to verify that our method converges with the predicted second order rate in time. To this end, we run the same single-body simulation from Figs.~\ref{01bodseq} and \ref{shrink_intface} (except with a coarser $N_\iin=128$) to a stopping time of $t_s = 1E-2$. This stopping time is 56\% of the vanishing time and was chosen so that the body will have eroded substantially, but is not so small that accuracy is lost. We assess error by measuring the difference between simulations of successive temporal resolution (i.e.~we verify Cauchy convergence). In particular, we measure the $L^2$-difference of the body shapes obtained at the stopping time, $t_s$, while successively halving the timestep $\Delta t$. Table \ref{convtab} shows the $L^2$-errors between successive simulations, along with the order of accuracy as obtained by Richardson-extrapolation-type calculations. The table confirms that our method to converges with second-order accuracy. In the table, we also show the CPU time of the simulation run on a single processor (though the algorithm is easily parallelized).
