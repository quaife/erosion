...
With the numerical method validated, we are now ready to simulate the erosion of multiple bodies. First, though, we will make one important conceptual change to the model. For a realistic porous medium, the condition of a fixed incoming flow rate, $\umax$, is somewhat artificial. In groundwater flow, for example, it is more realistic to have an upstream (recharge) region of given hydraulic head that connects to a downstream (discharge) region of another head. In our simulations, we will therefore impose the condition that the pressure difference across the domain remains fixed.

To impose the condition of a fixed pressure difference, we will dynamically control the incoming flow rate, $\umax$. To this end, we first compute the Stokes flow with $\umax = 1$ prescribed and measure the corresponding pressure at an upstream ($x = -2$) and downstream ($x=2$) location. We define the vertically-averaged upstream and downstream pressures as
\begin{align}
...
%Briefly, to verify that these rescaled solutions achieve the same order of accuracy, we report a convergence test in Table \ref{convtab}. In 
