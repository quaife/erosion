...
\end{equation}
Simpler version for my talk
\begin{align}
& \td{L}{t} = - \mean{\pd{\theta}{\alpha} V_n} \\
& \pd{\theta}{t} = \eps L^{-2} \ppd{\theta}{\alpha} + L^{-1} \left( \pd{\atau}{\alpha} + \pd{\theta}{\alpha} V_s \right)
\end{align}
...
Some numerical tests reveal that for 2D Stokes flow the stress scales like $\tau \sim 1/(L \log L)$, instead of the naive $1/L$. This is because we are in a 2D bounded domain (Stokes paradox type stuff). So we need to modify the curvature driven flow to have the same scaling. To this end, modify Eq.~(\ref{Vn1}) to read
\begin{equation}
V_n = \atau + \eps f(L) \left(\kappa - \frac{2 \pi}{L} \right)
\end{equation}
For 2D Stokes flow, we have 
\begin{equation}
f(L) = \frac{1}{\log (W/L)}
\end{equation}
where $W$ is the lengthscale of the 3rd dimension (e.g. the length of a long cylinder). The 2D approximation requires $W \gg L$, and in the code I chose $W = 2 \pi$. Carrying through this change, Eq.~(\ref{Vn2}) becomes
\begin{equation}
