...
The above total resistivity reflects the resistivity from the porous matrix as well as the channel walls. There is a buffer region that lies outside the porous-matrix region ($x_0$ lies in the buffer region), and we would like to remove the contribution from this region. Lets regard the total resistivity as an area-integral of resistivity-density, so that we can write the total in terms of the areas of the regions
\begin{equation}
...
\end{equation}
Where we have used $\qavg = \frac{2}{3} U$. These are the permeability and resistivity for the entire region $-x_0 < x < x_0$. In particular, the resistivity of the porous matrix will be averaged with the much lower permeability of the buffer region. To remove the contribution from the buffer region, and thus isolate the resistivity due to the porous matrix of bodies, we can simply set $x_0 = 1$ above. With also setting $w=1$, we get
\begin{equation}
\label{resdrag}
r_{tot} = \frac{3}{8} \frac{F_D}{U}
\end{equation}
With this simplification, I see a match between measuring resistivity via the drag \eqref{resdrag} versus measuring resistivity directly via \eqref{rtot} and \eqref{resmatrix}.
