...
\end{align} 
\todo[inline]{What is $x_0$ in the denominator? Should that be -1?}
...
Similar statistical analysis is performed on the tortuosity measurements and shown in Fig.~\ref{fig6}(a)--(b). Much like before, the tortuosity measurements are well separated by the number of bodies, with the error bars showing little overlap across different values of $M$. Measurements of the tortuosity ratio, $(T_2 -1)/(T_1 -1)$, however, show similar behavior across different values of $M$, as seen in Fig.~\ref{fig6}(c). For nearly all runs, the tortuosity ratio increases as bodies erode up to a maximum of roughly $(T_2 -1)/(T_1 -1) = 8$, and then the tortuosity ratio decreases near the very end of the simulation. As before the tortuosity ratio can be decomposed into configurational and shape components, as shown in Figs.~\ref{fig6}(d)--(e). The figure shows that the shape component is the main contribution, and the configurational component of the tortuosity ratio is minimal.
...
In this paper, we have introduced a Cauchy formulation of the boundary integral equation method to simulate the fluid-mechanical erosion of many bodies in a Stokes flow. The accuracy and efficiency of the method enables high-fidelity simulations of dense suspensions of $O(100)$ bodies with statistical analysis across an ensemble of different initializations. By linking the governing Stokes equation to Darcy's law, we are able to extract porous media properties, like permeability, resistivity, tortuosity, and anisotropy, as they evolve over time. By comparing the resistivity to the directly computed drag, we find that these porous-media properties can be extracted with high accuracy.

	The ratio of longitudinal to transverse permeability provides the main 



% Could reference Eastham paper on including chemistry, precipitation reaction.
% Could discuss convection in conjunction with erosion.
