...
\section{Introduction}
	...
	Figure \ref{fig1} shows an example simulation in which 80 circular bodies of randomized sizes and positions are immersed in a Stokes flow that moves from left to right. Color depicts the local speed of the flow intervening between bodies to give a sense of where the flow is strongest. Over time, individual bodies erode in response to the shear stresses induced on them.
The heterogeneous material removal creates visible features both at the level of individual bodies and the larger-scale configuration. For example, horizontally-oriented channels are clearly visible in the third and fourth frames.
	
%which regions of high flow speed create higher local shear rates, thus preferentially removing solid material, and further opening the channels. The positive feedback creates the channels that are clearly visible by in final two images.
