...
equations~\eqref{eqn:StokesEq}, we reformulate the system as a boundary
integral equation (BIE). We represent the velocity as the sum of a
double-layer potential and a combination of Stokeslets and
...
complex variables. With this reformulation, simple quadrature rules
achieve spectral accuracy. We identify $\xx = (x_1,x_2)$ with $x = x_1 +
ix_x$, and use similar notation for $\yy$, $\nn$, and $\eeta$. We also
interpret $\Gamma$, the boundary of the domain, as a collection of
Jordan curves in $\CC$. We next introduce the functions
\begin{align}
  \tau_1(y) = \eta(y) \overline{n(y)} \Re(n(y)), \quad 
  \tau_2(y) = \eta(y) \overline{n(y)} \Im(n(y)).
\end{align}
Then, we next define the five Cauchy integrals
\begin{align}
...
\end{align}
The boundary data of each of these Cauchy integrals satisfies the
Sokhotski-Plemelj jump relation
\begin{align}
  \lim_{\substack{x \rightarrow x_0 \\ x \in \Omega}} \int_{\Gamma}
    \frac{\eta(y)}{x-y}\, dy = -\frac{1}{2}\sigma(x_0) - 
