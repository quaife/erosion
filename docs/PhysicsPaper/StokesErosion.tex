...
To apply the aforementioned spectrally accurate quadrature rule, we
require the boundary data of the five Cauchy integrals. For $x_0 \in
\gamma$, the boundary data of each of these Cauchy integrals satisfies
the Sokhotski-Plemelj jump relation
\begin{align}
  \lim_{\substack{x \rightarrow x_0 \\ x \in \Omega}} \int_{\gamma}
    \frac{\eta(y)}{x-y}\, dy = -\frac{1}{2}\eta(x_0) - 
    \frac{1}{2\pi i} \int_{\gamma} \frac{\eta(y)}{x-y} \, dy,
    \quad x_0 \in \bd\Omega,
    \label{eqn:SP}
\end{align}
where the last integral is interpreted in the principal value sense. For
the Cauchy integral $v_i$, we let $v_i^-$ denote the boundary data that
satisfies~\eqref{eqn:SP}. Once $v_i^-$ is calculated, $v(x)$ and its
derivatives can be determined by the Cauchy Integral Theorem
\begin{subequations}
  \label{eqn:cauchy}
  \begin{alignat}{3}
  v(x) &= \frac{1}{2\pi i}\int_{\bd\Omega} 
