...
\section{Introduction}
	...
	Figure \ref{fig1} shows an example simulation of the erosion of 80 bodies, with the local speed of the intervening fluid flow shown by color. This simulation illustrates the {\em channelization} process, in which regions of high flow speed create higher local shear rates, thus preferentially removing solid material, and further opening the channels. The positive feedback creates the channels that are clearly visible by in final two images.
...
\caption{Numerical simulation of a dense suspension of 80 bodies eroding in a Stokes flow.
Over time, the non-uniform erosion of material opens channels in the medium, leading to high anisotropy.}
\end{figure*}
 %^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^%
