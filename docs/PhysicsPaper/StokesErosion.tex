...
\begin{equation}
r = \left( \frac{\pup - \pdn}{2 \qavg} \right)
\end{equation}
...
The resistivity is directly related to the drag exerted by the bodies. Indeed, another way to express the Stokes equations is $\grad \cdot \bvec{\sigma} = 0$ where $\bvec{\sigma} = -p \bvec{I} + \mu \left( \grad \bu + \grad \bu^T \right)$ is the stress tensor. The divergence theorem gives
\begin{equation}
\label{divthm}
...
\begin{equation}
\FD = (D_1, D_2) = -\int_{\gamma} \bvec{\sigma} \cdot \nhat \, dS 
\end{equation}
which includes both pressure and viscous drag. The minus sign is due to the orientation of the normal vector. We will mostly be interested in the horizontal component $D_1$.

Meanwhile, the horizontal component of the other term is
\begin{align}
\ex \cdot \int_{\Gamma} \bvec{\sigma} \cdot \nhat \, dS = 
