...
\label{eqn:erosionModel}
  ...
  \end{split}
\end{equation}
Here $\uu$ is the fluid velocity, $p$ is the pressure, \nick{\st{$\UU$ is a
prescribed Hagen-Poiseuille velocity field}, $\UU$ will be far-field uniform flow $\UU = (U,0)$, where the value $U = U(t)$ is adjusted to keep the pressure drop constant in time,}
%
and $\Vn$ is the normal velocity of $\gamma$. Since the rate of erosion is much slower than the background flow, a quasi-steady approximation justifies the no-slip boundary condition. Parameters include the fluid viscosity $\mu$ and the material-dependent erosion constant $\CE$. By non-dimensionalizing equations~\eqref{eqn:erosionModel}~\citep{qua-moo2018}, we set both these parameters to one. The shear stress on $\gamma$ is
\begin{align}
  \tau = -(\nabla \uu + \nabla \uu^T) \nn \cdot \ss
...
The boundary integral on the right-hand-side of \eqref{divthm} has contributions from the boundaries of the erodible bodies $\gamma$ and from the outer boundary $\Gamma$, since $\partial \Omega = \gamma \cup \Gamma$. In particular, the total drag exerted by the erodible bodies is given by
\begin{equation}
